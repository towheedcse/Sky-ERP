...
{ 
	//alert(catagory);
	  if(catagory!="")
	  {
		  httpLoadProduct.open("GET", 'index.php?app=purchase_order&cmd=loadProduct&catagory_id='+catagory, true);
...
function getSalesOrderInfo(order_no)
{ 
	  if(order_no!="")
	  {
		  httpLoadPO.open("GET", 'index.php?app=sales&cmd=loadSOInfo&po_no='+order_no, true);
...
function handlePOInfoResponse()
{
    ...
    {       
	   ...
	   document.getElementById('voucher_no').value=poinfoArr[0];
	   //document.getElementById('po_no').value = trim(poinfoArr[1]);
	   document.frmbuyerorder.catagory.value=trim(poinfoArr[2]);
	   var catagory = trim(poinfoArr[2]);
	   catagoryArr = catagory.split("###");	   
	   getProductList(catagoryArr[0])
    }    

}
//=========== End Get Purchase Info =====
//=========== Start Get Product Unit Price =====

function getUnitPrice(product_id)
{ 
...
function handleUPResponse()
{
    ...
    {       
	   ...
	   var uniteInfo 	 = trim(httpLoadUP.responseText);
	   
	   uniteInfoArr = uniteInfo.split("#####");
	   document.getElementById('unit_price').value=uniteInfoArr[0];
	   //document.getElementById('qty').value=uniteInfoArr[1];
	   document.getElementById('m_unit').value=uniteInfoArr[2];
	   getStockQty();
    }    
...
{ 
	  ...
	  if(product_id!="")
	  {
		  httpLoadSQ.open("GET", "index.php?app=sales&cmd=get_stock_qty&product_id="+product_id+"&voucher_no="+voucher_no, true);
...
function formSetup(frm)
{
	with(frm)
	{
		...
		setRequiredField(po_no,			     'textbox',  'wo_no_lbl');
