...
{ 
	  ...
	  if(product_id!="")
	  {
		  httpLoadPS.open("GET", "index.php?app=fg.production&cmd=load_stock&product_id="+product_id+"&transfer_stock="+transfer_stock, true);
...
function formSetup(frm)
{
	with(frm)
	{
		...
		setRequiredField(finish_qty,    	 'textbox',	 'production_qty');
...
function processDataGrid(catagoryid,catagoryname,brandid,brandname,catagory_product_id,productid,productname,pvoucher_no,m_unit,qty,currency,currencyName,amount,total_value)
{
	...
	tr.appendChild(addCol(m_unit+'<input type=hidden name=input_m_unit['+catagory_product_id+'] id=input_m_unit['+catagory_product_id+'] value='+m_unit+'><input type=hidden name=input_pvoucher_no[] id=input_pvoucher_no['+catagory_product_id+'] value='+pvoucher_no+'>'));
...
{		
	 ...
	 var finish_qty = parseInt(document.frmbuyerorder.finish_qty.value);
	  //alert(total_value);alert(discount);
	 if(out_qty=='' || out_qty==0){ out_qty = 0; }
	 if(!RE_DECIMAL.exec(finish_qty)){ highlightTableColumn('production_qty');}
	 
	 var qty = ((out_qty/1000) * finish_qty);  
