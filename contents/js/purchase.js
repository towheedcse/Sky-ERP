...
{ 
	  ...
	  if(brand_id!="")
	  { 	
		  httpLoadProduct.open("GET", "index.php?app=purchase&cmd=loadProduct&brand_id="+brand_id+"&catagory_id="+catagory_id, true);
...
{ 
	  ...
	  if(product_id!="")
	  {
		  httpLoadUP.open("GET", "index.php?app=purchase&cmd=getproductdtl&product_id="+product_id, true);
...
function handleUPResponse()
{
    ...
    {       
	   ...
	   var content 	 = trim(httpLoadUP.responseText);
	   contentArr	 = content.split("###");
	   document.getElementById('m_unit').value=contentArr[0];
	   var product_catagory=contentArr[1];  var details=contentArr[2];
	   document.getElementById('details').value=details;
	   if(product_catagory=="Serial"){
		   document.getElementById('serial').disabled =false;document.getElementById('warranty').disabled =false;
		   document.getElementById('qty').value=1; document.getElementById('qty').disabled =true;
	   }else{
		   document.getElementById('serial').disabled =true;document.getElementById('warranty').disabled =true;
...
{	
		...
		if(serial==""){ serial = 0; }
		catagory_product_id 	= catagoryid+'###'+brandid+'###'+productid+'###'+serial;
		//alert(catagory_product_id);
		podtl_id 	= document.getElementById('pur_detail_id').value;
		...
		if(rsFound)
		{
			processDataGrid(podtl_id,catagoryid,catagoryname,brandname,catagory_product_id,productid,productname,pdetails,serial,warranty,m_unit,unit_price,qty,total_bag,total_unit,currency,currencyName,total_value);
			rsFound = false;
...
function processDataGrid(podtl_id,catagoryid,catagoryname,brandname,catagory_product_id,productid,productname,pdetails,serial,warranty,m_unit,unit_price,qty,total_bag,total_unit,currency,currencyName,total_value)
{
	...
	tr.appendChild(addCol(catagoryname+'<input type=hidden name=input_catagory_product_id[] id=catagory_product_id[] value='+catagory_product_id+'>'));
  	...
  	tr.appendChild(addCol(productname+'<input type=hidden name=input_product[] id=input_product['+catagory_product_id+'] value='+productid+'><input type="hidden" name="input_pdetails['+catagory_product_id+']" id="input_pdetails['+catagory_product_id+']" value="'+pdetails+'">'));
...
{		
	 ...
	 if(isNaN(total_value)){ total_value = 0; } if(isNaN(discount)){ discount = 0;}
