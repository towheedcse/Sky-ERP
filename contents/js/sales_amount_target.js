...
function getAmountSalesTargetGride(){
	...
	if(date_from !="" && date_to!="" ) { 	
	  ...
	  httpLoadCSTGride.open("GET", "index.php?app=sales_target&cmd=loadAST&date_from="+date_from+"&date_to="+date_to+"&division_id="+division_id+"&district="+district_id+"&area="+area_id, true);
...
function handleLoadCSTGrideResponse()
{
    if(httpLoadCSTGride.readyState == 4){       
		...
		document.getElementById('tbs').innerHTML = JSON.parse(tbl);
...
function formSetup(frm)
{
	...
	{		
		...
		setRequiredField(target_amount,    'textbox',  'target_amount_lbl');
	}
}
...
{	
	...
	//alert(productid);	
	if(division_id !="" && districtId !="" && date_from !="" && date_to !=""){ 	
	 httpSaveCatagory.open("GET","index.php?app=sales_target&cmd=save_ast&division_id="+division_id+"&date_from="+date_from+"&date_to="+date_to+"&district="+districtId+"&area="+areaId+"&target_amount="+target_amount, true);
