...
function loadUndeliveryOrder(){
	...
	{	
		httpLoadUDP.open("GET", "index.php?app=sales.order&cmd=get_undelivery&customer_id="+customer_id+"&voucher_no="+voucher_no+"&delivery_point="+delivery_point+"&sales_date="+sales_date+"&delivery_date="+delivery_date, true);
...
function showOrverInvoice(response){
	...
	if(response && response.agingDate && response.agingDate != ""){
...
{	
	...
	httpSaveProduct.open("GET","index.php?app=sales.order&cmd=save_tmp&tmp_id="+tmp_id+"&customer="+customer+"&delivery_point="+delivery_point+"&sales_date="+sales_date
