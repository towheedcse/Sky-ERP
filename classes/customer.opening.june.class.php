...
class CustomerOpeningJune
{
  ...
  function showEditor()
  {
	 ...
	 if(getRequest('save')){
		$comApp->updateRecord(SUB_ACC_HEAD_TBL,"sub_id",$sub_id,"","","","","customer.opening.june","list");
		$msg="Successfully Update Record !!!";
		header("location:?app=customer.opening.june&cmd=list&msg=$msg");	      	
	 } 
	} else {		
	if(getRequest('save')) {
		mysql_query("START TRANSACTION;");
		//$Acc_id = $comApp->NewID(SUB_ACC_HEAD_TBL,"sub_id","A000001","A",7);
		//$comApp->saveRecord(SUB_ACC_HEAD_TBL,"sub_id",$Acc_id,"","","created_by","created_date","customer.opening.june","");
   ...
   function deleteItem(){
	...
	$comApp = new Common(); 
	$sub_id = getRequest('id');
	$comApp->deleteRecord(SUB_ACC_HEAD_TBL,"sub_id",$sub_id,"customer.opening.june","list"); 
   }  
} // End class
?>