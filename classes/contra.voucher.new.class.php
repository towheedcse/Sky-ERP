...
class ContraVoucher
{
   ...
   {         
      ...
      if( ($u_t_id == 101) || ($u_t_id == 102) || ($u_t_id == 104)) //1 = admin 2 = Sales man
      {
	   ...
	   case 'voucher_entry'         : $screen = $this->showEditorVoucherEntry($msg); break;
      	   case 'edit'               	: $screen = $this->showEditor("Edit Page");    break;	
      	   case 'print_vouchar'         : $screen = $this->showPrintEditor($msg); break;	
      	   case 'print_pending_vouchar' : $screen = $this->showPendingPrintEditor($msg); break;
	   ...
	   case 'save_vouchar'		: $this->saveVoucherEntry(); break;
	   ...
	   case 'get_due_voucher' 	: $this->getDueInvoiceList(); break; 
	   case 'voucher_bulk_import' 	: $this->VoucherBulkImport(); break; 
	   case 'voucher_account_info' 	: $this->getAccountInfo(); break;
  ...
  function showPrintEditor($msg = null) {   	  
    ...
    {
        $tmp_grvid = getRequest('tmp_grvid');
        if ($tmp_grvid) {
            $advArr = $this->getPendingContraMasterInfo($tmp_grvid);
            $advArr = parseThisValue($advArr);
            $data = array_merge(array(), $advArr);
            $data['item_list'] = $this->getPendingContraDetails($tmp_grvid);
            $data['message'] = $msg;
            $data['cmd'] = getRequest('cmd');
            require_once(PRNIT_PENDING_CONTRA_VOUCHER_SKIN);
            return true;
        }
    }
            ...
            if ($resMaster) {
                ...
                if (mysql_num_rows($gres) > 0) {
                    while ($row = mysql_fetch_object($gres)) {
                        ...
                        $requestDetailsData['tmp_grvid'] = $contra_id;
                        $requestDetailsData['project_id'] = $project_id;
                        $requestDetailsData['headtypes'] = $row->headtypes;
                        $requestDetailsData['dr_account'] = $row->dr_account;
                        $requestDetailsData['currency'] = $row->currency;
                        $requestDetailsData['currencyName'] = $row->currencyName;
                        $requestDetailsData['cr_account'] = $row->cr_account;
                        $requestDetailsData['cr_acname'] = $row->cr_acname;
                        $requestDetailsData['cr_amount'] = $row->cr_amount;
                        $requestDetailsData['bank_name'] = $row->bank_name;
                        $requestDetailsData['acc_no'] = $row->acc_no;
                        $requestDetailsData['check_no'] = $row->check_no;
                        $requestDetailsData['check_issue_date'] = $row->check_issue_date;
                        $requestDetailsData['cheque_type'] = $row->cheque_type;
                        $requestDetailsData['vouchar_type'] = getRequest('vouchar_type');
                        $requestDetailsData['consignee'] = $row->consignee;
                        $requestDetailsData['consignee_name'] = $row->consignee_name;

                        if ($row->dealer_payment == 1) {
                            $requestDetailsData['description'] = "MR No. " . $row->description;
                        } else {
                            $requestDetailsData['description'] = $row->description;
                        }
                        $requestDetailsData['created_by'] = $created_by;
                        $requestDetailsData['dealer_payment'] = $row->dealer_payment;
                        $requestDetailsData['due_invoice'] = $row->due_invoice;
                ...
                $snapshotdata['voucher_snapshot'] = json_encode($voucher_snapshot);

                $info = array();
                $info['table'] = PENDING_CVMASTER_TBL;
                $info['data'] = $snapshotdata;
                $info['where'] = "tmp_grvid ='$contra_id'";
                update($info);
   ...
   function approvedVoucher()
    {
        ...
        if ($ref_id > 0) {
	    $contra_id = $this->saveVoucher($ref_id);
	    if($contra_id){
    ...
    function approvedMoneyRecipt()
    {
        ...
        if ($ref_id > 0) {
	    $contra_id = $this->saveVoucher($ref_id, true);
	    if($contra_id){
		header("location:index.php?app=contra.voucher.new&cmd=print_vouchar&contra_id=$contra_id");
        	exit();
	    }
    ...
    function saveVoucher($tmp_grvid, $moneyReceipt = false)
    {
		...
		if ($moneyReceipt && $grow->status == 1) {
		   return $grow->contra_id;
		}
		...
		if ($grow->cr_account == "") {
            ...
            $_SESSION['transaction_name'] = "";
                ...
                if ($grow->dr_account != "") {
            		...
            		if ($vouchar_type == "Payable Vouchar") {
			    $_SESSION['vouchar_type'] = "Payment Vouchar";
                	    $_SESSION['transaction_name'] = "Payable";
                    	}
                    	if ($vouchar_type == "Recievable Vouchar") {
			    $_SESSION['vouchar_type'] = "Received Vouchar";
                            $_SESSION['transaction_name'] = "Recievable";
                    	}

                        if ($vouchar_type == "Journal Vouchar") {
                            $_SESSION['transaction_name'] = "Journal";
                        }
			if ($vouchar_type == "Contra Vouchar") {
	                    $_SESSION['transaction_name'] = "Contra";
	                }
                    }
                }
		...
		$contra_id = $this->saveContraMasterVouchar($voucher_no, $grow);
                $this->saveContraDetailsVouchar($contra_id, $voucher_no, $tmp_grvid, $grow, $moneyReceipt, $_SESSION['transaction_name'], $drHeadTypes);
                mysql_query("COMMIT;");
	...
	if (ob_get_level()) ob_end_clean();
 	...
 	$project_id = getFromSession('project_id');
        ...
        foreach ($finalArray as $row) {
            ...
            $conditions = array();
            if ($sub_id !== "") {
                $conditions[] = "BINARY sub_id = '$sub_id'";
            }
            if ($code !== "") {
                $conditions[] = "BINARY code = '$code'";
            }
            $whereExtra = '';
            if (!empty($conditions)) {
       ...
       $sql = "SELECT sub_id,code FROM " . SUB_ACC_HEAD_TBL . " WHERE project_id='$project_id' $whereExtra";

                //$sql = "SELECT sub_id,code FROM " . SUB_ACC_HEAD_TBL . " WHERE BINARY sub_id = '$sub_id' OR BINARY code = '$code'";
                $prow = mysql_fetch_object(mysql_query($sql));
                if ($prow->sub_id) {
		    $account_id = $prow->sub_id;
		    $accountData['account_id'] = $account_id;
		    $accountData['account_code'] = $prow->code;

                    $response['data'][$account_id] = $accountData;
                }
   ...
   function VoucherBulkImport()
    {
        ...
        $validation = true;
        foreach ($input['rows'] as $index => $row) {
            $voucher_id = trim($row['voucher_id']);
            if ($voucher_id == "") {
                $validation = false;
                $response['failed'][] = [
                    'index' => $index,
                    'error' => "Voucher ID must be fillable"
                ];
            }
        }
	    ...
	    $finalData = [];
            ...
            foreach ($input['rows'] as $index => $row) {
                ...
                if (!isset($finalData[$voucher_id])) {
                    ...
                    foreach ($masterFields as $field) {
                        ...
                        if ($value !== '') {
                            $finalData[$voucher_id]['master'][$field] = $value;
                        }
                    }
                ...
                foreach ($row as $key => $value) {
                    ...
                    if (!in_array($key, $masterFields) && $key !== 'voucher_id') {
                        $detailRow[$key] = $value;
                    }

                    if ($key == "account_type" && $value != "" && strtolower($value) == "dr") {
                        $finalData[$voucher_id]['master']['head_type'] = $value;
                        $finalData[$voucher_id]['master']['dr_account'] = $row['account'];
                        $finalData[$voucher_id]['master']['totaldr_ammount'] += (float)$row['amount'];
                    }

                    if ($key == "account_type" && $value != "" && strtolower($value) == "cr") {
                        $finalData[$voucher_id]['master']['cr_account'] = $row['account'];
                        $finalData[$voucher_id]['master']['totalcr_ammount'] += (float)$row['amount'];
                    }
                }
                
                // Add only if there's any detail data
                if (!empty($detailRow)) {
                    $finalData[$voucher_id]['details'][] = $detailRow;
                }
            }
            ...
            foreach ($finalArray as $row) {
                ...
                $voucher_type = trim($master['voucher_type']);
                $payment_mode = trim($master['payment_mode']);
                $adjustment_voucher = trim($master['adjustment_voucher']);
                $beddebts_voucher = trim($master['beddebts_voucher']);
                $bank_journal = trim($master['bank_journal']);
		$voucher_date = $this->formatCustomDate(trim($master['voucher_date']));
                $note = trim($master['note']);
                $head_type = trim($master['head_type']);
                $dr_account = trim($master['dr_account']);
                $cr_account = trim($master['cr_account']);
                $totaldr_ammount = trim($master['totaldr_ammount']);
                $totalcr_ammount = trim($master['totalcr_ammount']);


                if ($voucher_type == "" || $payment_mode == "" || $adjustment_voucher == "" || $beddebts_voucher == "" || $bank_journal == "" || $voucher_date == "") {
                    $masterFailedValidation = true;
                }

		$master['voucher_type'] = ucfirst($voucher_type);
                $master['payment_mode'] = ucwords($payment_mode);
                $master['head_type'] = ucfirst($head_type);
                $master['bank_journal'] = ucfirst($bank_journal);

                if (strtolower($adjustment_voucher) == "yes") {
                    $master['adjustment_voucher'] = 1;
                } else {
                    $master['adjustment_voucher'] = 0;
                }

                if (strtolower($beddebts_voucher) == "yes") {
                    $master['beddebts_voucher'] = 1;
                } else {
                    $master['beddebts_voucher'] = 0;
                }

                $validationErr = "";

                if ($dr_account == "" || $head_type == "") {
                    $masterFailedValidation = true;
                    if (empty($validationErr)) {
                        $validationErr = "Account and Head Type Fillable";
                    }
                }

                if (!in_array(strtolower($head_type), ['dr', 'cr'])) {
                    $masterFailedValidation = true;
                }

                if (!in_array(strtolower($bank_journal), ['yes', 'no'])) {
                    $masterFailedValidation = true;
                }

            
		if (abs($totaldr_ammount - $totalcr_ammount) > 0.05) {
                    $masterFailedValidation = true;
                    if (empty($validationErr)) {
                        $validationErr = "DR And CR Amount is not match! DR is ($totaldr_ammount) and CR is ($totalcr_ammount)";
                    }
                }
		...
		$processNext = false;
                ...
                foreach ($details as $dRow) {
                    ...
                    if ($masterFailedValidation) {
                        if (empty($validationErr)) {
                            $validationErr = "Voucher common correct data must be fillable any of the row";
                        }
                        $response['failed'][] = [
                            'index' => $index,
                            'error' => $validationErr
                    ...
                    $account_type = trim($dRow['account_type']);
                    $account = trim($dRow['account']);
                    $amount = trim($dRow['amount']);
			...
			$processNext = false;
                        $response['failed'][] = [
                            'index' => $index,
                            'error' => "Voucher Amount/Account/Account Type must be fillable"
                        ];
                        continue;
                    }else{
			$processNext = true;
		...
		if ($processNext) {
                    $result = $this->saveBulkVoucherData($master, $details);
    ...
    function saveBulkVoucherData($master = [], $details = [])
    {
        require_once(CLASS_DIR . '/common.list.class.php');
        $clistApp = new CommonList();
        $voucher_no = $this->createVoucharID(false);
        ...
        if ($voucher_no != "") {
            ...
            $vouchar_type = $master['voucher_type'];
            $totaldr_ammount = $master['totaldr_ammount'];
            $totalcr_ammount = $master['totalcr_ammount'];
            $payment_mode = $master['payment_mode'];
            $created_date = $this->formatCustomDate(trim($master['voucher_date']));
            $bank_journal = $master['bank_journal'];
            $bank_journal = $master['bank_journal'];
            $masterNote = $master['note'];

            $dr_amount = $totaldr_ammount;
            $cr_amount = $totalcr_ammount;
            ...
            $requestMasterData = getUserDataSet(CONTRA_MASTER_TBL);
	    $requestMasterData['dr_account'] = $master['dr_account'];
            $requestMasterData['cr_account'] = $master['cr_account'];
            $requestMasterData['dr_amount'] = $totaldr_ammount;
            $requestMasterData['cr_amount'] = $totalcr_ammount;
            $requestMasterData['mode_of_payment'] = $payment_mode;
            $requestMasterData['bank_journal'] = $bank_journal;
            $requestMasterData['currency'] = 1;
            $requestMasterData['voucher_no'] = $voucher_no;
            $requestMasterData['created_date'] = $created_date;
            $requestMasterData['project_id'] = getFromSession('project_id');
            $requestMasterData['created_by'] = getFromSession('userid');
            $requestMasterData['vouchar_type'] = $_SESSION['vouchar_type'];
            $requestMasterData['transaction_name'] = $_SESSION['transaction_name'];
            $requestMasterData['description'] = $masterNote;
            $info = array();
            ...
            if($contra_id){
                ...
                foreach ($details as $row) {
                    ...
                    $requestdata['project_id'] = getFromSession('project_id');
                    $requestdata['headtypes'] = $row['account_type'];
                    if ($row['account_type'] == "Dr") {
                        $requestdata['dr_account'] = $row['account'];
                    }
                    if ($row['account_type'] == "Cr") {
                        $requestdata['cr_account'] = $row['account'];
                    }
                    $requestdata['cr_amount'] = $row['amount'];
                    $requestdata['currency'] = 1;
                    $requestdata['bank_name'] = $row['bank'];
                    $requestdata['acc_no'] = $row['branch'];
                    $requestdata['check_no'] = $row['cheque'];
                    $requestdata['check_issue_date'] = $this->formatCustomDate(trim($row['issue_date']));
                    $requestdata['cheque_type'] = "Cash Deposit";
                    $requestdata['vouchar_type'] = $_SESSION['vouchar_type'];
                    $requestdata['consignee'] = $row['retailer'];
                    $requestdata['transaction_name'] = $_SESSION['transaction_name'];
                    $requestdata['description'] = $row['details'];
                    //$requestdata['due_invoice'] = $row['due_invoice'];
                    $requestdata['created_by'] = getFromSession('userid');
                    ...
                    if ($res) {
                        ...
                        $issue_date = $this->formatCustomDate(trim($row['issue_date']));
                        $vouchar_type = $_SESSION['vouchar_type'];
                        $transaction_type = $_SESSION['transaction_name'];
                        $project_id = getFromSession('project_id');

                        //$paid_invoice = $row['due_invoice'];

                        if ($row['retailer'] != "") {
                            $consignee = $row['retailer'];
                            $CrAmount = $row['amount'];
                        ...
                        } elseif ($row['account_type'] == "Cr") {
                            //========= Cr Account ========
                            $cr_account = $row['account'];
                            $CrAmount = $row['amount'];
   ...
   function saveVoucherEntry($moneyReceipt = false){	
	if (ob_get_level()) ob_end_clean();
	...
	$this->createVoucherEntry($voucher_no,$_SESSION['vouchar_type'],$dr_amount);
	$contra_id 	= $this->saveContraMasterVoucharEntry($voucher_no);

	$cost_center = getRequest('cost_center');	
	$this->saveContraDetailsVoucharEntry($contra_id,$voucher_no,$cost_center); 
	mysql_query("COMMIT;");
	}
	if($contra_id!=""){ 
	echo $contra_id;
   ...
   function saveContraMasterVouchar($voucher_no, $master)
	...
	$requestdata['edited_by'] = $master->edited_by;
   ...
   function saveContraMasterVoucharEntry($voucher_no){
	$requestdata = array();
	$requestdata = getUserDataSet(CONTRA_MASTER_TBL);
	$requestdata['dr_amount'] 		= getRequest('totaldr_amount');
    ...
    function createVoucherEntry($voucher_no,$vouchar_type,$dr_amount){
	// ===Start Create Voucher===
    ...
    function saveContraDetailsVouchar($contra_id, $voucher_no, $tmp_grvid, $master, $moneyReceipt, $transaction_name, $drHeadTypes)
    {
        require_once(CLASS_DIR . '/common.list.class.php');
        $clistApp = new CommonList();
        $requestdata = array();
        ...
        if (mysql_num_rows($gres) > 0) {
            while ($row = mysql_fetch_object($gres)) {
                ...
                $requestdata['created_by'] = $row->created_by;
                $requestdata['due_invoice'] = $row->due_invoice;
		...
		$cost_center = isset($master->cost_center) ? $master->cost_center : "";
		    ...
		    $created_by = $master->created_by;
                    ...
                    if ($row->consignee != "") {
                        ...
                        $totalDR = $this->getTotalDebitAmount($consignee, getFromSession('project_id'));
                        $balance = ($totalDR - ($totalCR + $CrAmount));
                        $this->saveRetailerJournal($voucher_no, $consignee, $transaction_type, $project_id, $description, 0, $CrAmount, $balance, 1, $created_date, $issue_date, $created_by);
                        $CrAmount = 0;
                        $balance = 0;
                    }

// Due invoice code here..
                    if (!empty($paid_invoice)) {
                        if (strpos($paid_invoice, 'SI') === 0) {
                            $sql = "SELECT voucher_no, net_payble, paid_amount, due FROM " . SALES_MASTER_TBL . " WHERE voucher_no ='$paid_invoice'";
                            $smrow = mysql_fetch_object(mysql_query($sql));

                            if (!empty($smrow->voucher_no) && $row->cr_amount > 0) {
                                $voucherPaidAmount = (string)$row->cr_amount;

                                // keep as string (important)
                                $updateVoucher = $smrow->voucher_no;
                                $netPayble = $smrow->net_payble;
                                $paidAmount = $smrow->paid_amount;

                                // precise calculation
                                $updatedPaidAmount = bcadd($paidAmount, $voucherPaidAmount, 2);
                                $updatedDueAmount = bcsub($netPayble, $updatedPaidAmount, 2);

                                // prevent negative due
                                if (bccomp($updatedDueAmount, '0', 2) < 0) {
                                    $updatedDueAmount = '0.00';
                                }

                                $SMUpdate = "UPDATE " . SALES_MASTER_TBL . " 
                                             SET paid_amount='$updatedPaidAmount',
                                                 due='$updatedDueAmount' 
                                             WHERE voucher_no='$updateVoucher' 
                                             AND project_id='$project_id'";

                                mysql_query($SMUpdate);
                            }

                        } elseif (strpos($paid_invoice, 'INV') === 0) {
                            $sql = "SELECT voucher_no, net_payable, paid_amount, due FROM bill WHERE voucher_no ='$paid_invoice'";
                            $smrow = mysql_fetch_object(mysql_query($sql));
                            ...
                            if (!empty($smrow->voucher_no) && $row->cr_amount > 0) {
                                ...
                                // paid status check
                                $paid_status = 0;
                                if (bccomp($updatedPaidAmount, $netPayble, 2) >= 0) {
                                    $paid_status = 1;
                                }

                                $BillUpdate = "UPDATE bill 
                                   SET paid_amount='$updatedPaidAmount',
                                       due='$updatedDueAmount',
                    ...
                    if ($row->headtypes == "Dr") {
                        ...
                        $totalPartyDR = $this->getTotalDebitAmount($dr_account, getFromSession('project_id'));
                        $PartyBalance = (($totalPartyDR + $DrAmount) - $totalPartyCR);
                        $this->saveAccountJournal($voucher_no, $dr_account, $transaction_type, $project_id, $description, $DrAmount, 0, $PartyBalance, 0, $created_date, $issue_date, $adjustment, $beddebts, $created_by,$cost_center,$paid_invoice);
                        //=========== Cr Capital =======
                        $HeadType = getHeadType($dr_account);
                        if ($HeadType == "Administrative Cost") {
                            $capital_head = $clistApp->getCapitalId(getFromSession('project_id'));
                            if ($capital_head) {
                                ...
                                $totalCapitalDR  = $this->getTotalDebitAmount($capital_head,getFromSession('project_id'));
                                $Capitalbalance  = ($totalCapitalDR-($totalCapitalCR+$DrAmount));
                                $this->saveAccountJournal($voucher_no,$capital_head,$transaction_type,$project_id,$description,0,$DrAmount,$Capitalbalance,0,$created_date, $adjustment, $beddebts, $created_by);
                                */
                            }

                        }
                        //if($vouchar_type!="Payable Vouchar" && $vouchar_type!="Recievable Vouchar"){
                        $HeadType = $drHeadTypes;
                        if ($HeadType != "Cash" || $HeadType != "Bank") {
                            require_once(CLASS_DIR . '/advanced_payment.class.php');
                            $advpApp = new AdvancedPayment();
                            $head_type = getHeadType($dr_account);
                        ...
                        $totalDR = $this->getTotalDebitAmount($cr_account, getFromSession('project_id'));
                        $balance = ($totalDR - ($totalCR + $CrAmount));
                        $this->saveAccountJournal($voucher_no, $cr_account, $transaction_type, $project_id, $description, 0, $CrAmount, $balance, 0, $created_date, $issue_date, $adjustment, $beddebts, $created_by,$cost_center,$paid_invoice);
				...
				$numbers = explode(",", $recipients);
                        ...
                        if ($collection_source != "Others") {
                            ...
                            if ($capital_head) {
                                ...
                                $this->saveAccountJournal($voucher_no,$capital_head,$transaction_type,$project_id,$description,$CrAmount,0,$Capitalbalance,0,$created_date, $adjustment, $beddebts, $created_by);
                                */
                            }
        ...
        if ($res) {
            if ($moneyReceipt) {
		$updateData['contra_id'] = $contra_id;
                $updateData['status'] = '1';
                $info = array();
                $info['table'] = PENDING_CVMASTER_TBL;
    ...
    function saveContraDetailsVoucharEntry($contra_id,$voucher_no,$cost_center=""){	
	 require_once(CLASS_DIR.'/common.list.class.php');	
	 $clistApp = new CommonList();
	 $requestdata 				= array();	
	 ...
	 if(mysql_num_rows($gres)>0){
	  while($row = mysql_fetch_object($gres)){
		...
		if($res){
			...
			$totalPartyDR  = $this->getTotalDebitAmount($dr_account,getFromSession('project_id'));
			$PartyBalance  = (($totalPartyDR+$DrAmount)-$totalPartyCR);
			$this->saveAccountJournal($voucher_no,$dr_account,$transaction_type,$project_id,$description,$DrAmount,0,$PartyBalance,0,$created_date,$issue_date,NULL,NULL,NULL,$cost_center);	
			//=========== Cr Capital =======
			$HeadType 		  = getHeadType($dr_account);  
			if($HeadType=="Administrative Cost"){
			 $capital_head    = $clistApp->getCapitalId(getFromSession('project_id'));
			...
			$totalDR   = $this->getTotalDebitAmount($cr_account,getFromSession('project_id'));
			$balance   = ($totalDR-($totalCR+$CrAmount));					 
			$this->saveAccountJournal($voucher_no,$cr_account,$transaction_type,$project_id,$description,0,$CrAmount,$balance,0,$created_date,$issue_date,NULL,NULL,NULL,$cost_center);
  ...
  function sendSMS($sender,$recipients,$message){	
   ...
   function showEditor($msg = NULL) { 
	...
	$data['invoice_list']   = $this->getDueInvoices();
	$data['message'] = $msg;
	$data['cmd']     = getRequest('cmd');
	...
	$search = isset($_POST['search']) ? trim($_POST['search']) : "";
        ...
        $result = $this->getDueInvoices($search);
...
function getDueInvoices($search = "")
{
    $overdue_invoice = getFromSession('overdue_invoice');
    $overdue_bill = getFromSession('overdue_bill');

    if($overdue_invoice == 0 && $overdue_bill == 1){
        return $this->getDueBillInvoices($search);
    }
    ...
    $info = array();
    $info['table']  = SALES_MASTER_TBL . " m
                       INNER JOIN " . SALES_DELIVERY_MASTER_TBL . " d
                       ON d.voucher_no = m.voucher_no";
    $info['fields'] = array('m.voucher_no');
    $whereSQL  = "m.due > 0 AND m.project_id = '$project_id'";
    if (!empty($search)) {
        $search = addslashes($search);
        $whereSQL .= " AND m.voucher_no LIKE '%$search%'";
    }
    $info['where']  = $whereSQL;
    $info['orderby'] = array("m.voucher_no ASC LIMIT 0 , 50");
...
function getDueBillInvoices($search = "")
{
    $project_id = getFromSession('project_id');
    $data = array();
    ...
    $info = array();
    $info['table'] = "bill";
    $info['fields'] = array('voucher_no');
    $whereSQL = "paid_status != 1 AND due > 0 AND project_id = '$project_id'";

    if (!empty($search)) {
        $search = addslashes($search);
        $whereSQL .= " AND voucher_no LIKE '%$search%'";
    }
    $info['where']  = $whereSQL;
    $info['groupby'] = array("bill_id");
    $info['orderby'] = array("bill_id ASC LIMIT 0 , 50");
    //$info['debug']  = "true";
   ...
   function showEditorVoucherEntry($msg = NULL) { 
        require_once(CLASS_DIR.'/common.list.class.php');	
	$clistApp 	= new CommonList(); 
	$ref_id   	= getRequest('ref_id');
	$project_id 	= getFromSession('project_id');
	...
	$data['message'] = $msg;
	$data['cmd']     = getRequest('cmd');
	require_once(TEMPLATES_SKINS . '/contra.voucher.voucher_entry.html');      
   ...
   {     
	...
	$requestdata['due_invoice'] 	= getRequest('due_invoice');
	$requestdata['currency'] 	= getRequest('currency');
	$requestdata['currencyName'] 	= getRequest('currencyName');
	...
	*/
	...
	if ($sub_id && $sub_id != "") {
		$getSql = "SELECT cost_center_required FROM " . SUB_ACC_HEAD_TBL . " WHERE sub_id='$sub_id'";
		$subResult = mysql_fetch_object(mysql_query($getSql));
		if (isset($subResult->cost_center_required) && $subResult->cost_center_required == 1) {
	...
	if($due_invoice !=""){
		$due_invoice = "<br>Invoice : $due_invoice";
	}
	$str2.="
	<tr style='color:#000000' bgcolor='#CCCCCC'>
	  <td width='25%' align='left' nowrap>$cr_acname $consignee_name $due_invoice</td>
	  <td width='6%' align='left' nowrap>$headtypes</td>
	  <td width='10%' align='right' nowrap>$cr_amount $currencyName</td>
	  <td width='11%' align='center' nowrap>$acc_no</td>
	  <td width='11%' align='center' nowrap>$branch_name</td>
	  ...
	  <td width='10%' align='left' nowrap>$check_issue_date</td>
	  <td width='12%' align='left' nowrap>$description</td>				  
	  <td width='5%' align='center' nowrap><div class='table-option'>
	...
	$str3="</table>";
	$total_salesStr = $str1.$str2.$str3."####-@@@@".$totalCr_amount."####-@@@@".$totalDr_amount."####-@@@@" . $costCenterRequired;
	...
	$account = $row->cr_account;
	if($row->headtypes == "Dr"){
	   $account = $row->dr_account;
	}
	$str = $row->tmp_id."#####".$row->headtypes."#####".$account."#####".$row->consignee."#####".$row->cr_amount."#####".$row->bank_name."#####".$row->acc_no."#####".$row->check_no."#####".dateInputFormatDMY($row->check_issue_date)."#####".$row->description."#####".$row->due_invoice;
	echo $str;
    }
    //======= End SaveIn Temp Tbl ========     
	...
	$info['fields'] = array('cm.contra_id','cm.voucher_no','cm.headtypes','cm.adjustment','cm.beddebts','cm.dr_account','p.project_name','p.location','p.project_logo','cm.dr_amount','cm.mode_of_payment','cm.vouchar_type','cm.transaction_type','cm.description',"DATE_FORMAT(cm.created_date,'%d %b %y' ) as created_date",'c.curr_symble','cm.created_by','cm.created_time','cm.is_money_recipt');	
    ...
    function getPendingContraMasterInfo($id)
    {
        ...
        $info['fields'] = array('cm.tmp_grvid', 'cm.headtypes', 'cm.adjustment', 'cm.beddebts', 'cm.dr_account', 'p.project_name', 'p.location', 'p.project_logo', 'cm.dr_amount', 'cm.mode_of_payment', 'cm.vouchar_type', 'cm.transaction_type', 'cm.description', "DATE_FORMAT(cm.created_date,'%d %b %y' ) as created_date", 'c.curr_symble', 'cm.created_by', 'cm.created_time' , 'cm.is_money_recipt');
   ...
   function getPendingContraDetails($id)
    {
        ...
        $info['fields'] = array('cd.tmp_id', 'cd.tmp_grvid', 'cd.headtypes', 'cd.dr_account', 'cd.cr_account', 'cd.cr_amount', 'cd.bank_name', 'cd.acc_no',
            'cd.check_no', "DATE_FORMAT(cd.check_issue_date,'%d %b %y' ) as check_issue_date", 'cd.cheque_type', 'cd.vouchar_type', 'c.curr_symble', 'cd.description', 'cd.created_by');
  ...
  function saveAccountJournal($voucher_no,$sub_id,$transaction_type,$project_id,$description,$DR=NULL,$CR=NULL,$balance,$status,$created_date,$issue_date, $adjustment = NULL, $beddebts = NULL, $created_by = NULL,$cost_center="",$paid_invoice=""){
	...
	$sql = "INSERT INTO ".ACCOUNT_JOURNAL_TBL." (voucher_no,created_date,issue_date,sub_id,head_type,transaction_type,project_id,description,adjustment,beddebts,dr,cr,balance,status,created_by,cost_center,paid_invoice) 
	 VALUES('".$voucher_no."','".$created_date."','".$issue_date."','".$sub_id."','".$head_type."','".$transaction_type."','".$project_id."','".$description."','".$adjustment."','".$beddebts."','".$DR."','".$CR."','".$balance."','".$status."','".$created_by."','" . $cost_center . "','" . $paid_invoice . "')";
	mysql_query($sql);
  }

  function saveRetailerJournal($voucher_no,$sub_id,$transaction_type,$project_id,$description,$DR=NULL,$CR=NULL,$balance,$status,$created_date,$issue_date=NULL, $created_by = NULL){
	$head_type  = "Retailer";   
	if (!isset($created_by)) {
            $created_by = getFromSession('userid');
        }
   ...
   // ======== Create Voucher ID =======
	...
	if ($user_sl != "") {
            ...
            $info['where'] = "transaction_type = 'Contra Voucher' AND voucher_no LIKE '" . $prefix . "0%'";
        }
