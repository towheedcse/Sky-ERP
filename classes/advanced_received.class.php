   ...
   function showEditor($msg = null) { 
    ...
    function saveCreditVouchar($voucher_no)
 	{     
		  ...
		  if($res['affected_rows']) {
			...
			$HeadType = getHeadType(getRequest('cr_account'));  
			if($mode_of_payment=="Cash"){ 
			 ...
			 $this->saveAccountJournal($voucher_no,$acc_head,"Cash",getFromSession('project_id'),getRequest('description'),$DrAmount,0,$balance,0,$created_date);	
			 //======= Party Cr ======	
			 ...
			 $this->saveAccountJournal($voucher_no,$PartyAcc_head,$HeadType,getFromSession('project_id'),getRequest('description'),0,$CrAmount,$PartyBalance,0,$created_date);
			...
			header("location:index.php?app=advanced_received&cmd=add");	
		  }
    }//EOFn 
	...
	function saveAccountJournal($voucher_no,$sub_id,$head_type,$project_id,$description,$DR=NULL,$CR=NULL,$balance,$status,$created_date){
		...
		$sql = "INSERT INTO ".ACCOUNT_JOURNAL_TBL." (voucher_no,created_date,sub_id,head_type,transaction_type,project_id,description,dr,cr,balance,status,created_by)
		 VALUES('".$voucher_no."','".$created_date."','".$sub_id."','".$head_type."','Received','".$project_id."','".$description."','".$DR."','".$CR."','".$balance."','".$status."','".$created_by."')";
		mysql_query($sql);
	}
