  ...
  function showEditor()
  {
		 ...
		 {		 			
			 ...
			 {  
        		...
        		header("location:?app=party&cmd=view&msg=$msg");		      	
			 } 
	  ...
	  $data                		= array();
	  $data['party_list']  	= array($this->getRecords(getRequest('from'),getRequest('to')));       
   ...
   {       
	  $requestdata = array();
      $requestdata = getUserDataSet(PARTY_TBL); 
      ...
      if(!$res) {
        header("location:?app=party&cmd=view");
      }               
   ...
   function getRecords($from=null,$to=null)
   {
	   ...
	   $info['table']   = PARTY_TBL; 
	   if($party_id !=""){    
       	 $info['where']  =  "party_id='".$party_id."' AND project_id ='$project_id'";
	   }elseif($srckey !=""){    

       	 $info['where']  =  "party_id = '".$srckey."' OR name like '".$srckey."%' OR mname like '".$srckey."%'  AND project_id ='$project_id'";
  ...
  function deleteRecord($id)
  {
      ...
      { 
          ...
          header("location:?app=party&cmd=view&msg=$msg");     	   
      		 ...
      		 header("location:?app=party&cmd=view&cmd=list&deleted=no");
      	}      	
   ...
   function createID()
   {
      $info = array();
      $info['table'] = PARTY_TBL;
      $info['fields'] = array('max(party_id) as maxparty');
      ...
      $maxpartyId = generateID("P",$maxpartyId,7);
      return $maxpartyId;
   }   
   ...
   function getDistrictList()
   {
    ...
    {	  
	   $party_id = getFromSession('party_id');
  	   $data            = array();	  
  	   $info            = array();
	   $info['table']   = PARTY_TBL.' a,'.USER_TBL.' u';
...
function viewProfileEditor($msg = null) { 
  ...
  function userEditor()
  {
		 ...
		 {		 			
			 ...
			 {  
				...
				$msg="Successfully Update Record !!!";
        		header("location:?app=party&cmd=view_user&msg=$msg");		      	
			 } 
   ...
   {       
      ...
      if(!$res) {
        header("location:?app=party&cmd=view_user");
      }               
  ...
  function bankDepositEditor()
  {
		...
		if(getRequest('save')) {
			...
			$msg="Successfully Save Record !!!";
			header("location:?app=party&cmd=bank_deposit&msg=$msg");	     		       		      	
		 }			 
   ...
   {       
      ...
      if($res)
      {
       	 header("location:?app=party&cmd=bank_deposit");
      }        
