		   ...
		   foreach($arr_unit_price as $key => $val)
			...
			//dumpvar($info);	     
			 ...
			 if(getRequest('salse_type')=="Sales"){
				 ...
				 $this->saveStockJournal($voucher_no,$project_id,$product_id,$requestdata['unit_price'],$requestdata['m_unit'],0,$productQty,$balance,$sales_date);
		...
		if($mode_of_payment =="Check"){
		  ...
		  if($res['affected_rows']) {
			 ...
			 if($mode_of_payment=="Cash"){ 
				...
				//====== save payable_check ======
				$this->savePayableCheck($voucher_no,"Received",getRequest('paid_amount'));
