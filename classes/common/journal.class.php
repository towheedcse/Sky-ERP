		  ...
		  if($res['affected_rows']) {
			 ...
			 if($mode_of_payment=="Cash"){ 
     	     ...
     	     if(getRequest('pref_no')!="" && getRequest('dir')=="purchase"){
				...
				$this->saveAccountJournal($voucher_no,$pblAcc_head,"Acc",getFromSession('project_id'),getRequest('description'),0,$payble,$pblBalance,1,$date);	//before 0
			
				$psql= "SELECT net_payble, paid_amount ,due FROM ".PURCHASE_MASTER_TBL." WHERE voucher_no  = '".getRequest('pref_no')."' AND due >0";
				...
				if(mysql_num_rows($pres)>0){
					...
					$total_due = $net_payble - ($paidAmount + $DrAmount);
					$total_paid = ($paidAmount + $DrAmount);
					$pusql= "UPDATE  ".PURCHASE_MASTER_TBL." SET paid_amount ='".$total_paid."', due ='".$total_due."' WHERE voucher_no  = '".getRequest('pref_no')."' AND due >0";
					$pures = mysql_query($pusql);
					header("location:index.php?app=journal&cmd=vouchar_print&voucher_no=".$voucher_no);
				}
			}
			
			if(getRequest('cref_no')!="" && getRequest('dir')=="PAC"){
				$cref_no = getRequest('cref_no');
				//========= Capital Cr ==========	
				$CrAmount		 = getRequest('debit');
				...
				$this->saveAccountJournal($voucher_no,$CptAcc_head,"Acc",getFromSession('project_id'),getRequest('description'),0,$CrAmount,$CptBalance,1,$date);	//before 0
				...
				if(mysql_num_rows($pres)>0){
					...
					$pusql= "UPDATE ".SALES_MASTER_TBL." SET commission_total_paid ='".$total_paid."',commission_total_due ='".$total_due."' WHERE voucher_no = '".$cref_no."' AND commission_total_due >0 AND commission_status=1";
					$pures = mysql_query($pusql);
					header("location:index.php?app=journal&cmd=vouchar_print&voucher_no=".$voucher_no);
				}
			}
...
//============= Start Payable check ============
	...
	function depositAmount($voucher_no){
		$rsql= "SELECT acc_head, net_payble, paid_amount ,due, acc_no FROM ".PAYABLE_CHECK_TBL." WHERE voucher_no  = '".$voucher_no."' AND status=0 AND transaction_type='Received'";
		$rres = mysql_query($rsql);
		if(mysql_num_rows($rres)>0){
			$srow = mysql_fetch_object($rres);						
			$due = $srow->due;
			...
			$acc_head	= $srow->acc_head;
			//============= journal ==========
			if($net_payble>$paidAmount){
				 //=========== Receivable Dr ========
				 $fullReceivable		= $net_payble;
				...
				//============== Bank Account Dr ===============
				 ...
				 $this->saveAccountJournal($voucher_no,$acc_head,"Acc",getFromSession('project_id'),getRequest('description'),$paidAmount,0,$balance,1);	
