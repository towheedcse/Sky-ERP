...
class SalesReturn{
  ...
  $PMsql = "SELECT voucher_no,net_payble,paid_amount,return_amount,due,item_delivery_amount FROM ".SALES_MASTER_TBL." WHERE customer ='".$customer."' AND project_id = '$project_id' 
  AND paid_amount < net_payble AND due >0";
  ...
  {  	
	...
	for($j; $j<=$totalfields; $j++){
		...
		$RSQL="INSERT INTO ".SALES_RETURN_TBL."(voucher_no,sales_details_id,project_id,customer_id,catagory,brand_id,product_id,currency,return_qty,
		sales_unit_price,return_date,return_by) VALUES('$voucher_no','$sales_details_id','$project_id','$customer_id','$catagory','$brand_id','$product_id','$currency',
		'$return_qty','$sales_unit_price','$return_date','$return_by')";
		mysql_query($RSQL);
		}// update sales dtl
	}//  end for	
  ...
  function getDueSalesReturnPaybleList($from,$to) { 
		...
		$info['fields'] = array('sr.return_id','sr.voucher_no','sr.project_id','p.project_name','p.location','sr.customer_id','s.sub_head_name','s.head_details',"DATE_FORMAT(sr.return_date,'%d %b %y' ) as return_date",'sr.return_amount','c.curr_symble','sr.paid_amount','sr.due as return_due');
		...
		if($cr_account!="All" && $cr_account!="Customer" && $cr_account!="Supplier"){
			$sql.=" AND sr.customer_id = '$cr_account' ";
		}
   ...
   function getDueSupplierPaybleList($from,$to) { 
		if($from == "" && $to == ""){$from=0; $to=700;}
		$date_from 		= formatDate(getRequest('date_from'));
		$date_to 		= formatDate(getRequest('date_to'));				
		$project_id     = getFromSession('project_id');  
		$info           = array();    
		$info['table']  = SALES_RETURN_PAYBLE_TBL.' sr,'.SUPPLIER_TBL.' s,'.PROJECT_TBL.' p,'.CURRENCY_TBL.' c';	
		$info['fields'] = array('sr.return_id','sr.voucher_no','sr.project_id','p.project_name','p.location','sr.customer_id','s.sub_head_name','s.head_details',"DATE_FORMAT(sr.return_date,'%d %b %y' ) as return_date",'sr.return_amount','c.curr_symble','sr.paid_amount','sr.due as return_due');
		if(getFromSession('project_type')!='Group Company'){
		$sql="sr.customer_id = s.supplier_code AND sr.project_id = p.project_id AND sr.currency = c.currency_id AND sr.project_id = '".$project_id."' AND sr.due>0 ";
		}else{
		$sql="sr.customer_id = s.supplier_code AND sr.project_id = p.project_id AND sr.currency = c.currency_id AND sr.due>0 ";
		}
