...
class UsedForFeed
{
   ...
   */
   ...
   {         
      ...
      if(($u_t_id == 101) || ($u_t_id ==102) || ($u_t_id == 103) || ($u_t_id == 104)) // 1 = sysadmin, 2 = admin, 3 = project admin
      {
  ...
  function saveFinishProduction(){
	 ...
	 $used_date 		= formatDate(getRequest('finish_date'));					
	 $this->saveStockJournal($production_id,getFromSession('project_id'),$finish_product,$sold_cost,$m_unit,$finish_qty,0,$balanceF,$used_date);
	 $sql = "UPDATE ".PRODUCTION_MASTER_TBL." SET finish_qty = '$finish_qty', production_type='Finish', finish_date = '".$used_date."' WHERE production_id='".$production_id."'";
	 $res = mysql_query($sql);
	 header("location:index.php?app=used4_feed&cmd=pro_dtl"); 
  }
  function insertProductionDetails($production_id)
  {
		...
		if(getRequest('production_type')=="Finish"){
		 ...
		 $balanceF  = (($totalFDR+$finish_qty) - $totalFCR);	
		 $used_date = formatDate(getRequest('used_date'));					
		 $this->saveStockJournal($production_id,$project_id,$finish_product,$sold_cost,$m_unit,$finish_qty,0,$balanceF,$used_date);
		}
	...
	function getProductionMasterInfo($id){		
		...
		$info['fields'] = array('pm.production_id','pa.project_name','pa.location','pm.overhead_cost','pm.sold_cost','pm.unit_price','pm.sales_price','pm.total_value','pm.m_unit','p.product_name','p.product_desc','pm.finish_qty',"DATE_FORMAT(pm.used_date,'%d %b %y' ) as used_date",'pm.production_type','c.curr_symble','pm.created_time');
  ...
  function showEditor4ProductionDetails($msg = null) {        
	  ...
	  $data                				= array();
	  $data['cmd']         				= getRequest('cmd');
	  $data['record_list'] 				= $this->getProductionMasterList(getRequest('from'),getRequest('to'));
	  $data['totalrecord']				= $this->getTotalProductionMasterList();	
   ...
   function getProductionMasterList($from,$to){		
		if($from == "" && $to == ""){$from=0; $to=40;}
		$production_type = getRequest('production_type');  
		$project_id     = getFromSession('project_id');  
		$info           = array();    
		$info['table']  = PRODUCTION_MASTER_TBL.' pm,'.PROJECT_TBL.' pa,'.PRODUCT_TBL.' p,'.CURRENCY_TBL.' c';	
		$info['fields'] = array('pm.production_id','pm.finish_product','pa.project_name','pa.location','pm.overhead_cost','pm.sold_cost','pm.unit_price','pm.sales_price','pm.total_value','pm.m_unit','p.product_name','pm.finish_qty',"DATE_FORMAT(pm.used_date,'%d %b %y' ) as used_date",'pm.production_type','c.curr_symble','pm.created_time');
   ...
   function getTotalProductionMasterList() {  
