...
class SalesDelivery{
  ...
  function insertDeliveryChallanDetails($voucher_no,$dm_id)
  {
	...
	$totalDeliveryAmount = 0; $totalDeliveryQty = 0; $totalOrderQty = 0;$j=1;
		...
		if($stockQty>=$UDQty){
			...
			if(($stockQty >= $freeQty) && ($freeQty >=0)){
				...
				if($pdres){
					...
					$this->saveProfit($voucher_no,$product,$dm_id,$unit_profit,$NowDeliveryQty,$freeQty,$sales_price,$purchase_price,$overall_discount_amount,$delivery_date);
					}
