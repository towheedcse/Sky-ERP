...
class ContraVoucherEdit
{
	...
	if($sl>0){
	  while($i < $sl){
		...
		if($res){
			...
			$this->saveAccountJournal($voucher_no,$dr_account,$transaction_type,$project_id,$description,$DrAmount,0,$PartyBalance,1,$created_date,$issue_date);	
			...
			$balance  = ($totalDR-($totalCR+$CrAmount));					 
			$this->saveAccountJournal($voucher_no,$cr_account,$transaction_type,$project_id,$description,0,$CrAmount,$balance,1,$created_date,$issue_date);
  ...
  function  getTotalContraVoucherList(){
   ...
   function saveAccountJournal($voucher_no,$sub_id,$transaction_type,$project_id,$description,$DR=NULL,$CR=NULL,$balance,$status,$created_date,$issue_date=NULL){
