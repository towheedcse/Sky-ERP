...
class Division_model extends CI_Model {
	...
	function InsertRecord(){
		...
		if($division_id==""){
			...
			$this->db->insert(DIVISION_TBL, $data);
		}else{
			$this->EditRecord($division_id);
		}
		
		//print  $this->db->last_query();
   	}
	function EditRecord($division_id){
		$company_id         =$this->input->post('company_id');
		$division_name      =$this->input->post('division_name');
		$division_status    =$this->input->post('division_status');
		...
		$this->db->update(DIVISION_TBL, $data);
        }
	
	function DelRecord(){
		$id =$this->input->post('id');
		$this->db->where('id',$id);
		$this->db->delete(DIVISION_TBL);
	}
	...
	function FillRecord(){
   	...
   	function GetRecordGrid(){
		...
		$this->db->from(DIVISION_TBL." AS d");
		...
		'<table width="100%"  border="0" class="table table-responsive table-bordered table-hover custab">
			<thead>
			  <tr class="active">
			  	...
			  	<th width="32%">'.$this->lang->line("division").' '.$this->lang->line("of").' '.$this->lang->line("name").'</th>
			  ...
			  if($row->division_status==1){ $status=$this->lang->line("active");}elseif($row->division_status==0){ $status=$this->lang->line("inactive");}
