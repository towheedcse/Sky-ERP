...
class Module_model extends CI_Model {
	...
	function InsertRecord(){
		...
		$this->db->insert(MODULE_TBL, $data);
		}else{
			$this->EditRecord($module_id);
		}
		//print  $this->db->last_query();
   	}
	function EditRecord($module_id){
		$company_id	=$this->input->post('company_id');
		$module_name	=$this->input->post('module_name');
		$module_icon	=$this->input->post('module_icon');
		$order_no       =$this->input->post('order_no');
		...
		$this->db->update(MODULE_TBL, $data);
    	}
	
	function DelRecord(){
		$id =$this->input->post('id');
		$this->db->where('module_id',$id);
		$this->db->delete(MODULE_TBL);
	}
	
	function FillRecord(){
		$module_id	=$this->input->post('id');
		$this->db->select('*');
		$this->db->from(MODULE_TBL);
		$this->db->where('module_id', $module_id);
		$query = $this->db->get();
		return $query->row();
	}
		...
		'<table width="100%"  border="0" class="table table-responsive table-bordered table-hover custab">
			<thead>
			  <tr class="active">
			  	...
			  	<th width="27%">'.$this->lang->line("company_name").'</th>
				<th width="20%">'.$this->lang->line("module_name").'</th>
				<th width="20%">'.$this->lang->line("faicon").'</th>
                <th width="8%">'.$this->lang->line("order").'</th>
			  ...
			  if($row->module_status==1){ $status=$this->lang->line("active");}elseif($row->module_status==0){ $status=$this->lang->line("inactive");}
			  echo "<tr class='default'>
				...
				if($hasEditPM){
				echo "<span data-toggle='tooltip' data-placement='top' data-original-title='Edit'><a class='btn btn-info btn-xs' onclick=editRecord('".$row->module_id."') id='".$row->module_id."' href='#'><i class='fas fa-edit'></i></a></span>&nbsp;";
				}
				if($hasDelPM){
				echo "<span data-toggle='tooltip' data-placement='top' data-original-title='Delete'><a class='btn btn-danger btn-xs' data-toggle='modal' onclick=deleteRecord('".$row->module_id."') id='".$row->module_id."' data-target='#deleteModal'><i class='fa fa-trash'></i></a></span>";
				}
				echo "</td>
			  </tr>";
			  $i++;
	...
	function GetModulePermissionRecordGrid(){
		$company_id		=$this->input->post('company_id');
		$module_status		=$this->input->post('module_status');
		...
		if($module_status >0){
		$this->db->where('module_status',$module_status);
		}
		$this->db->group_by('module_id');
		$this->db->order_by('order_no','ASC');
		$this->db->limit($to,$from);
		$query = $this->db->get();//print  $this->db->last_query();
		$totalrecord = $this->GetTotalModulePermissionRecordGrid();
		$perPage=50; $Pagination="";
		if($totalrecord >0){
		$Pagination = $this->getPagination($totalrecord,$perPage);
		} //print  $this->db->last_query();
		...
		<thead>
		  <tr class='bg-light'>
		     ...
		     <th width='18%' rowspan='2' class='align-middle'>".$this->lang->line("module_name")."</th>
  		     <th width='80%' colspan='".$totalroles."' class='text-center'>".$this->lang->line("user_role")."</th>
		</tr>
		<tr class='bg-light'>
		";
		if($totalroles >0){
		...
		if($totalroles >0){
			foreach($rquery->result() as $rrow){
				...
				$CSQL = "SELECT * FROM ".MODULE_PERMISSION_TBL." WHERE company_id=".$company_id." AND module_id=".$module_id." AND role_id=".$role_id;
				...
				$grid.= "<td class='text-center'><input  type='checkbox' $checked onclick=savePermission('".$module_id."','".$role_id."',this) $disabled /></td>";
			}
		}
		$grid.= "</tr>";
		$i++;
	...
	function GetTotalModulePermissionRecordGrid(){
		$company_id		=$this->input->post('company_id');
		$module_status	=$this->input->post('module_status');
		if(empty($company_id)){$company_id = $this->session->userdata('company_id');} 
		if(empty($module_status)){$module_status=0;}
		...
		$this->db->group_by('module_id','ASC');
		$this->db->order_by('order_no','ASC');
		$query = $this->db->get();
		if($query->num_rows() >0){
			return $query->num_rows();
	...
	function InsertPermissionRecord(){
		...
		if($action_type=='insert'){
		    ...
		    $this->db->insert(MODULE_PERMISSION_TBL, $data);
		}else{
		    $this->db->where('company_id',$company_id);
		    $this->db->where('module_id',$module_id);
		    $this->db->where('role_id',$role_id);
		    $this->db->delete(MODULE_PERMISSION_TBL);
		}
    	}
