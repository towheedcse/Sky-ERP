...
class Project_model extends CI_Model {
	...
	function InsertRecord(){
		...
		$projectPrice	    	=$this->input->post('project_price');
		$projectBudget	    	=$this->input->post('project_budget');
   ...
   function GetRecordGrid(){
	...
	$this->db->from(PROJECT_TBL." AS p");
	$this->db->join(BRANCH_TBL.' AS b', 'b.id=p.branch_id','LEFT');
	...
	'<table width="100%"  border="0" class="table table-responsive table-bordered table-hover custab">
	   <thead>
            <tr class="active">
                ...
                <th width="15%">'.$this->lang->line("company_name").'</th>
                <th width="12%">'.$this->lang->line("branch_name").'</th>
                <th width="15%">'.$this->lang->line("project_name").'</th>
                <th width="20%">'.$this->lang->line("project").' '.$this->lang->line("description").'</th>
                <th width="10%">'.$this->lang->line("project_price").'</th>
                <th width="10%">'.$this->lang->line("project_budget").'</th>
		...
		echo "<tr class='default'>
			...
			if($hasDelPM){
			echo "<span data-toggle='tooltip' data-placement='top' data-original-title='Delete'><a class='btn btn-danger btn-xs'  data-toggle='modal' onclick=deleteRecord('".$row->id."') id='".$row->id."' href='#deleteModal'><i class='fas fa-trash'></i></a></span>";
			}
			echo "</td>
		    </tr>";
		    $i++;
	...
	function FillRecord(){
        $projects_id	=$this->input->post('id');
        $this->db->select('*');
        $this->db->from(PROJECT_TBL);
	...
	function EditRecord($projects_id){
        $companyName	    =$this->input->post('company_name');
        $branchName	        =$this->input->post('branch_name');
        $projectName	    =$this->input->post('project_name');
        $projectDescription	=$this->input->post('project_description');
