...
class Qualification_model extends CI_Model {
	...
	function InsertRecord(){
		...
		$qualificationName	=$this->input->post('qualification-name');
		$status			    =$this->input->post('status');
		$qualificationId    =$this->input->post('qualification-id');
		...
		$this->db->insert(QUALIFICATION_TBL, $data);
		}else{
			$this->EditRecord($qualificationId);
		}
   	...
   	function GetRecordGrid(){
		...
		$this->db->from(QUALIFICATION_TBL." AS s");
	  	$this->db->join(COMPANY_SETTINGS_TBL.' AS c', 'c.company_id=s.institute_id','LEFT');
		$this->db->group_by('s.qualification_id');
		$this->db->order_by('s.qualification_id','ASC');
		...
		'<table width="100%"  border="0" class="table table-responsive table-bordered table-hover custab">
			<thead>
			  <tr class="active">
				...
				<th width="20%">'.$this->lang->line("qualification_name").'</th>
			  ...
			  echo "<tr class='default'>
				...
				<td class='text-center align-middle'>";
		    		if($hasEditPM){
				echo "<span data-toggle='tooltip' data-placement='top' data-original-title='Edit'><a class='btn btn-info btn-xs' onclick=editRecord('".$row->qualification_id."') id='".$row->qualification_id."' href='#'><i class='fas fa-edit'></i></a></span>&nbsp;";
				}
				if($hasDelPM){				
				echo "<span data-toggle='tooltip' data-placement='top' data-original-title='Delete'><a class='btn btn-danger btn-xs' data-toggle='modal' onclick=deleteRecord('".$row->qualification_id."') id='".$row->qualification_id."' data-target='#deleteModal'><i class='fa fa-trash'></i></a></span>";
				}
			     	echo "</td>
			  </tr>";
			  $i++;
	...
	function DelRecord(){
		...
		$this->db->delete(QUALIFICATION_TBL);
	}
	
	function FillRecord(){
		$cl_id	=$this->input->post('id');
		$this->db->select('*');
		$this->db->from(QUALIFICATION_TBL);
		$this->db->where('qualification_id', $cl_id);
		$query = $this->db->get();
		return $query->row();
	}
	function EditRecord($qualification_id){
		$companyId		=$this->input->post('company-name');
		$qualificationName	=$this->input->post('qualification-name');
		$status			=$this->input->post('status');
		...
		$this->db->update(QUALIFICATION_TBL, $data);
    }
