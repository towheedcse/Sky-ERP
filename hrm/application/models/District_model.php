...
class District_model extends CI_Model {
	...
	function InsertRecord(){
		...
		if($district_id==""){
			...
			$this->db->insert(DISTRICT_TBL, $data);
		}else{
			$this->EditRecord($district_id);
		}
		
		//print  $this->db->last_query();
   	}
	function EditRecord($district_id){
		$company_id         =$this->input->post('company_id');
		$district_name      =$this->input->post('district_name');
		$status             =$this->input->post('status');
		...
		$this->db->update(DISTRICT_TBL, $data);
        }
	
	function DelRecord(){
		$id =$this->input->post('id');
		$this->db->where('id',$id);
		$this->db->delete(DISTRICT_TBL);
	}
	...
	function FillRecord(){
   	...
   	function GetRecordGrid(){
		...
		$this->db->from(DISTRICT_TBL." AS d");
			...
			<thead>
			  <tr class="active">
                ...
                <th width="38%">'.$this->lang->line("company_name").'</th>
			  	<th width="32%">'.$this->lang->line("district").' '.$this->lang->line("of").' '.$this->lang->line("name").'</th>
