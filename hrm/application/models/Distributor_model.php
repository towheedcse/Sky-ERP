...
class Distributor_model extends CI_Model {
	...
	function InsertAccountHead(){
		...
		$account_name		= $this->input->post('distributor_full_name');
		...
		$attention_person	= $this->input->post('attention_person');
	...
	function InsertRecord(){		
		$created_by			=$this->session->userdata('created_by');
		$distributorid		=$this->input->post('distributor_id'); 
		...
		$distributor_full_name	= $this->input->post('distributor_full_name');
		$billing_address	= $this->input->post('billing_address');
		$shipping_address	= $this->input->post('shipping_address');
		$present_address	= $this->input->post('present_address');
		$permanent_address	= $this->input->post('permanent_address');
		...
		if($distributorid==''){
			$distributor_id 		= $this->InsertAccountHead();
			$photo 					= $this->UploadPhoto($distributor_id);
			//$passport_attach	    = $this->UploadPassport($distributor_id);
			//$others_attach	        = $this->UploadOthers($distributor_id);
			if(empty($distributor_code)){
				$ssql = "SELECT head_id FROM ".ACC_HEAD_TBL." WHERE account_id = $distributor_id AND company_id = $institute_id AND branch_id = $branch_id";
			...
			'district'    		    =>$district,	
			'thana'    		        =>$thana,	
			'distributor_full_name' =>$distributor_full_name,
			...
			$this->db->insert(DISTRIBUTOR_TBL, $data);			
			$email = $this->input->post('email');
			$this->ManageUser($distributor_id,$institute_id,$branch_id,$login_id,$password,$email,1,$distributor_full_name,5,"I");			
		}else{			
			$email = $this->input->post('email');
			$this->EditRecord($distributorid,$institute_id,$branch_id);			
			$this->updateAccountHead($distributorid,$institute_id,$branch_id);
			$this->ManageUser($distributorid,$institute_id,$branch_id,$login_id,$password,$email,1,$distributor_full_name,5,"U");
		}		
		//print  $this->db->last_query();
   	}
	...
	function updateAccountHead($account_id,$institute_id,$branch_id){
			...
			$data = array(	
			'account_name'    	=>$distributor_full_name,
			'bangla_name'    	=>$distributor_full_name,
			'account_details' 	=>$billing_address,
			'permanent_address' =>$shipping_address,
	...
	function EditRecord($distributor_id,$institute_id,$branch_id){
		...
		$rkey				= mt_rand(10,99);
		$accesskey			= $password.$rkey;
		$distributor_photo  = $_FILES['distributor_photo'];
		if($distributor_photo!=""){
			$photo 			=$this->UploadPhoto($distributor_id);				
		}else{
			$ssql   = "SELECT photo FROM ".DISTRIBUTOR_TBL." WHERE distributor_id = $distributor_id AND company_id = $institute_id AND branch_id = $branch_id";
			$squery = $this->db->query($ssql);				
			if($squery->num_rows() >0){				   
		...
		$this->db->where('company_id',$institute_id);
		$this->db->where('branch_id',$branch_id);
		$this->db->update(DISTRIBUTOR_TBL, $data); //print  $this->db->last_query(); exit;
    }
	
	function ManageUser($distributor_id,$institute_id,$branch_id,$login_id,$password,$email,$status,$distributor_name,$user_role,$mode){
		$created_by	= $this->session->userdata('created_by');				
		$rkey		= mt_rand(10,99);
		...
		if($mode=="I"){		
		  ...
		  $SQL.="VALUES('".$distributor_id."','".$institute_id."','".$branch_id."','".$login_id."','".$password."','".$email."','".$status."','".$distributor_name."','".$access_key."','".$user_role."','".$created_by."')";
		  $this->db->query($SQL);
		}else{
		  $CSQL= "UPDATE ".USERS_TBL." SET company_id='".$institute_id."', branch_id='".$branch_id."', password='".$password."', email='".$email."', user_status='".$status."', display_name='".$distributor_name."', access_key='".$access_key."' WHERE ref_id='".$distributor_id."' AND user_name = '".$login_id."'";
		  $this->db->query($CSQL);
		}

	}    	
	function UploadPhoto($img_id){
	    if($_FILES['distributor_photo']){
            ...
            //$field = $_FILES['customer_photo'];
		  
            // No problems with the file
		
            // So lets upload 
            if ($this->upload->do_upload('distributor_photo')){ //print_r($field);
	...
	function FillRecord(){
		...
		$this->db->select('*');
		$this->db->from(DISTRIBUTOR_TBL);
		$this->db->where('status', 1);
		$this->db->where('distributor_id', $distributor_id);
		$query = $this->db->get();
		return $query->row();
	}		
	function GetAjaxDistributorList($distributor_id){		
		$PSQL= "SELECT * FROM ".ACC_HEAD_TBL." WHERE head_type IN(2) AND status=1 ";			
		$PSQL.= " GROUP BY account_id ORDER BY account_name ASC";
		$query = $this->db->query($PSQL);
		$options = "<option value='0'>".$this->lang->line('select')." ".$this->lang->line('distributor')."</option>";
   	...
   	function GetRecordGrid(){
		...
		$head_types = array(2);	
		$this->db->select('a.*,c.currency_name,p.account_id,p.group_id,p.subsidiary_level1,p.subsidiary_level2,p.head_type,p.account_name,p.nationality,p.mobile,p.email,p.gender,i.company_name,b.branch_name,b.branch_code',FALSE);
		$this->db->from(DISTRIBUTOR_TBL." AS a");
		$this->db->join(CURRENCY_TBL.' AS c', 'c.currency_id=a.currency','LEFT');
		$this->db->join(ACC_HEAD_TBL.' AS p', 'p.account_id=a.distributor_id','LEFT');
		...
		$this->db->group_by('a.distributor_id');
		$this->db->order_by('a.distributor_code','ASC');
		...
		'<table width="100%"  border="0" class="table table-responsive table-bordered table-hover custab">
			<thead>
			  <tr class="active">
				...
				<th width="20%">'.$this->lang->line("distributor").' '.$this->lang->line("details").'</th>
			  ...
			  echo "<tr class='default'>
				...
				<td><img src='".base_url().ASSETS.'/img/'.$row->photo."' height='30'/><br>".$row->account_name.",<br>A/C No.: ".$row->account_id."<br> Code: ".$row->short_code.", Currency: ".$row->currency_name."
				</td>
		    	...
		    	if($hasEditPM){
				echo "<span data-toggle='tooltip' data-placement='top' data-original-title='Edit'><a class='btn btn-info btn-xs' onclick=editRecord('".$row->distributor_id."') id='".$row->distributor_id."' href='#'><i class='fas fa-edit'></i></a></span>&nbsp;";
				}
				if($hasDelPM){				
				echo "<span data-toggle='tooltip' data-placement='top' data-original-title='Delete'><a class='btn btn-danger btn-xs' data-toggle='modal' onclick=deleteRecord('".$row->distributor_id."') id='".$row->distributor_id."' data-target='#deleteModal'><i class='fa fa-trash'></i></a></span>";
				}
				if($hasPrintPM){
				echo "<div class='clearfix'></div><br><span data-toggle='tooltip' data-original-title='View Form'><a class='btn btn-success btn-sm' target='_blank' href='".base_url()."distributor/ViewProfile/".$row->distributor_id."'><i class='fa fa-print'></i> Profile</a></span>";
				}
			    echo "</td>
			  </tr>";
			  $i++;
	...
	function DelRecord(){
		...
		$ESQL= "UPDATE ".DISTRIBUTOR_TBL." SET status='".$status."'WHERE distributor_id='".$distributor_id."'";
		$this->db->query($ESQL);
		
		$ASQL= "UPDATE ".ACC_HEAD_TBL." SET status='".$status."'WHERE account_id='".$distributor_id."'";
		$this->db->query($ASQL);
		
		$USQL= "UPDATE ".USERS_TBL." SET user_status='".$status."'WHERE ref_id='".$distributor_id."'";
		$this->db->query($USQL);
		/*
		//=====Start Delete ======
		$this->db->where('distributor_id',$distributor_id);
		$this->db->delete(DISTRIBUTOR_TBL); 
		
		$this->db->where('account_id',$distributor_id);
		$this->db->delete(ACC_HEAD_TBL);
		
		$this->db->where('ref_id',$distributor_id);
