...
class Shift_model extends CI_Model {
	...
	function InsertRecord(){
		...
		$shiftName		=$this->input->post('shift-name');
		$shiftStart     =$this->input->post('shift-start');
		$shiftEnd       =$this->input->post('shift-end');
		$shiftCapacity  =$this->input->post('shift-capacity');
		$status			=$this->input->post('status');
		$shiftId		=$this->input->post('shift-id');
		...
		if($shiftId==''){
			...
			$this->db->insert(SHIFT_TBL, $data);
		}else{
			$this->EditRecord($shiftId);
		}
   	...
   	function GetRecordGrid(){
		...
		<thead>
		  <tr class="active">
			...
			<th width="15%">'.$this->lang->line("shift_name").'</th>
			<th width="20%">'.$this->lang->line("shift_duration").'</th>
			<th width="13%">'.$this->lang->line("shift_capacity").'</th>
		  ...
		  echo "<tr class='default'>
			...
			<td>".$row->shift_name."</td>
                	...
                	<td>".$row->shift_capacity."</td>
			<td>".$status."</td>
			<td class='text-center align-middle'>";
	    		if($hasEditPM){
			echo "<span data-toggle='tooltip' data-placement='top' data-original-title='Edit'><a class='btn btn-info btn-xs' onclick=editRecord('".$row->shift_id."') id='".$row->shift_id."' href='#'><i class='fas fa-edit'></i></a></span>&nbsp;";
			}
			if($hasDelPM){				
			echo "<span data-toggle='tooltip' data-placement='top' data-original-title='Delete'><a class='btn btn-danger btn-xs' data-toggle='modal' onclick=deleteRecord('".$row->shift_id."') id='".$row->shift_id."' data-target='#deleteModal'><i class='fa fa-trash'></i></a></span>";
			}
		     	echo "</td>
			  </tr>";
			  $i++;
	...
	function FillRecord(){
		...
		$this->db->select('*');
		$this->db->from(SHIFT_TBL);
		$this->db->where('shift_id', $b_id);
		$query = $this->db->get();
		return $query->row();
	}
	function EditRecord($shift_id){
		$companyId	=$this->input->post('company-name');
		$shiftName	=$this->input->post('shift-name');
        	$shiftStart     =$this->input->post('shift-start');
        	$shiftEnd       =$this->input->post('shift-end');
