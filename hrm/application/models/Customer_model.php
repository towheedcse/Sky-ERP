...
class Customer_model extends CI_Model {
	...
	function InsertAccountHead(){
		...
		$account_name		= $this->input->post('customer_full_name');
		...
		$accountId		= $this->input->post('customer_id');
	...
	function InsertRecord(){		
		...
		if($customerid==''){
			$customer_id 			= $this->InsertAccountHead();
			$photo 					= $this->UploadPhoto($customer_id);
			//$passport_attach	    = $this->UploadPassport($customer_id);
			//$others_attach	        = $this->UploadOthers($customer_id);
			if(empty($customer_code)){
				$ssql = "SELECT head_id FROM ".ACC_HEAD_TBL." WHERE account_id = $customer_id AND company_id = $institute_id AND branch_id = $branch_id";
				$squery = $this->db->query($ssql);				
				if($squery->num_rows() >0){				   
				   $customer_code = $squery->row()->head_id;
				}
			}			
			$data = array(
			...
			$email = $this->input->post('email');
			$this->ManageUser($customer_id,$institute_id,$branch_id,$login_id,$password,$email,1,$customer_full_name,5,"I");			
		}else{			
			$email = $this->input->post('email');
			$this->EditRecord($customerid,$institute_id,$branch_id);			
			$this->updateAccountHead($customerid,$institute_id,$branch_id);
			$this->ManageUser($customerid,$institute_id,$branch_id,$login_id,$password,$email,1,$customer_full_name,5,"U");
		}		
		//print  $this->db->last_query();
   	}
	...
	function updateAccountHead($account_id,$institute_id,$branch_id){
			...
			$data = array(	
			'account_name'    	=>$customer_full_name,
			'bangla_name'    	=>$customer_full_name,
			'account_details' 	=>$billing_address,
			'permanent_address' =>$shipping_address,
			'nationality'     	=>$nationality,
			'father_name'    	=>$contact_person,
			...
			$this->db->update(ACC_HEAD_TBL, $data); //print  $this->db->last_query(); exit;
	}
	function EditRecord($customer_id,$institute_id,$branch_id){
		...
		$rkey				= mt_rand(10,99);
		$accesskey			= $password.$rkey;
		$customer_photo     = $_FILES['customer_photo'];
		if($customer_photo!=""){
			$photo 			=$this->UploadPhoto($customer_id);				
		}else{
			$ssql   = "SELECT photo FROM ".CLIENT_TBL." WHERE customer_id = $customer_id AND company_id = $institute_id AND branch_id = $branch_id";
			$squery = $this->db->query($ssql);				
			if($squery->num_rows() >0){				   
		...
		$this->db->where('company_id',$institute_id);
		$this->db->where('branch_id',$branch_id);
		$this->db->update(CLIENT_TBL, $data); //print  $this->db->last_query(); 
    }
	
	function ManageUser($customer_id,$institute_id,$branch_id,$login_id,$password,$email,$status,$customer_name,$user_role,$mode){
		$created_by	= $this->session->userdata('created_by');				
		$rkey		= mt_rand(10,99);
		...
		if($mode=="I"){		
		  ...
		  $SQL.="VALUES('".$customer_id."','".$institute_id."','".$branch_id."','".$login_id."','".$password."','".$email."','".$status."','".$customer_name."','".$access_key."','".$user_role."','".$created_by."')";
		  $this->db->query($SQL);
		}else{
		  $CSQL= "UPDATE ".USERS_TBL." SET company_id='".$institute_id."', branch_id='".$branch_id."', password='".$password."', email='".$email."', user_status='".$status."', display_name='".$customer_name."', access_key='".$access_key."' WHERE ref_id='".$customer_id."' AND user_name = '".$login_id."'";
		  $this->db->query($CSQL);
		}
	...
	function UploadPhoto($img_id){
	    if($_FILES['customer_photo']){
            ...
            // So lets upload 
            if ($this->upload->do_upload('customer_photo')){ //print_r($field);
	...
	function GetAjaxCustomerList($customer_id){		
		$PSQL= "SELECT * FROM ".ACC_HEAD_TBL." WHERE head_type IN(1,11) AND status=1 ";			
		$PSQL.= " GROUP BY account_id ORDER BY account_name ASC";
		$query = $this->db->query($PSQL);
		$options = "<option value='0'>".$this->lang->line('select')." ".$this->lang->line('customer')."</option>";
		...
		'<table width="100%"  border="0" class="table table-responsive table-bordered table-hover custab">
			<thead>
			  <tr class="active">
				...
				<th width="20%">'.$this->lang->line("customer").' '.$this->lang->line("details").'</th>
			  	<th width="20%">'.$this->lang->line("customer_address").'</th>
			  ...
			  echo "<tr class='default'>
		    	...
		    	if($hasEditPM){
				echo "<span data-toggle='tooltip' data-placement='top' data-original-title='Edit'><a class='btn btn-info btn-xs' onclick=editRecord('".$row->customer_id."') id='".$row->customer_id."' href='#'><i class='fas fa-edit'></i></a></span>&nbsp;";
				}
				if($hasDelPM){				
				echo "<span data-toggle='tooltip' data-placement='top' data-original-title='Delete'><a class='btn btn-danger btn-xs' data-toggle='modal' onclick=deleteRecord('".$row->customer_id."') id='".$row->customer_id."' data-target='#deleteModal'><i class='fa fa-trash'></i></a></span>";
				}
				if($hasPrintPM){
				echo "<div class='clearfix'></div><br><span data-toggle='tooltip' data-original-title='View Form'><a class='btn btn-success btn-sm' target='_blank' href='".base_url()."customer/ViewProfile/".$row->customer_id."'><i class='fa fa-print'></i> Profile</a></span>";
				}
			    echo "</td>
			  </tr>";
			  $i++;
	...
	function DelRecord(){
		...
		$ESQL= "UPDATE ".CLIENT_TBL." SET status='".$status."'WHERE customer_id='".$customer_id."'";
		$this->db->query($ESQL);
		
		$ASQL= "UPDATE ".ACC_HEAD_TBL." SET status='".$status."'WHERE account_id='".$customer_id."'";
		$this->db->query($ASQL);
		
		$USQL= "UPDATE ".USERS_TBL." SET user_status='".$status."'WHERE ref_id='".$customer_id."'";
		$this->db->query($USQL);
		/*
		//=====Start Delete ======
		$this->db->where('customer_id',$customer_id);
		$this->db->delete(CLIENT_TBL);
