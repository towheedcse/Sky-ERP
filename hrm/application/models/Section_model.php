...
class Section_model extends CI_Model {
	...
	function FillRecord(){
   	...
   	function GetRecordGrid(){
		...
		$this->db->select('s.*,c.company_name,d.department_name');
		$this->db->from(SECTION_TBL." AS s");
		$this->db->join(COMPANY_SETTINGS_TBL.' AS c', 'c.company_id=s.company_id','LEFT');
		$this->db->join(DEPARTMENT_TBL.' AS d', 'd.department_id=s.department_id','LEFT');
		$this->db->group_by('section_id','ASC');
		...
		<thead>
		  <tr class="active">
        	...
        	<th width="21%">'.$this->lang->line("company_name").'</th>
		  	<th width="22%">'.$this->lang->line("department").'</th>
		  	<th width="15%">'.$this->lang->line("section").'</th>
		  	<th width="16%">'.$this->lang->line("section_head").'</th>
		  ...
		  foreach($query->result() as $row){
		    ...
		    }elseif($row->section_status==0){ $status=$this->lang->line("inactive");}
			...
			if($hasEditPM){
			echo "<span data-toggle='tooltip' data-placement='top' data-original-title='Edit'><a class='btn btn-info btn-xs' onclick=editRecord('".$row->section_id."') id='".$row->section_id."' href='#'><i class='fas fa-edit'></i></a></span>&nbsp;";
			}
			if($hasDelPM){
			echo "<span data-toggle='tooltip' data-placement='top' data-original-title='Delete'><a class='btn btn-danger btn-xs' data-toggle='modal' onclick=deleteRecord('".$row->section_id."') id='".$row->section_id."' data-target='#deleteModal'><i class='fa fa-trash'></i></a></span>";
			}
			echo "</td>
		  </tr>";
		  $i++;
