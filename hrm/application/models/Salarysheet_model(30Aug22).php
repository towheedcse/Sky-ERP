...
class Salarysheet_model extends CI_Model {
        ...
        foreach($query->result() as $row){
			...
			if($CRES->num_rows() >0){
				...
				if($gross_deduction>0){
				   if($employee_id==251){ // Shahanaj Sultana
				    $cash_deduction         = (($CRES->row()->absent_fine/100)*40);
				   }elseif($employee_id==211){ // Zannatul Ferdus
				    $cash_deduction         = ($CRES->row()->loan);
				   }else{
				    $cash_deduction         = (($CRES->row()->absent_fine/100)*40);
				    $cash_deduction         = ($cash_deduction + $CRES->row()->loan);
				   }
				}else{
				 $cash_deduction         = 0;    
				}
				
				if($employee_id==209){ // Imran
				   $cash_deduction         = 0;
				}
				if($employee_id==251){
				  $loan_deduction 		= 0;  
			...
			if($CRES->num_rows()>0 && $CRES->row()->bill_id >0 && $CRES->row()->status ==2 ){
				...
				if($employee_id==209){ // Imran
				 $cash_absent_deduction = 0;
				}elseif($employee_id==211){ // Zannatul Ferdus
				 $cash_absent_deduction =0; 
				}else{
				 $cash_absent_deduction = (($absent_deduction/100)*40);   
				}
    ...
    function GetBankSalarySheetRecordGrid(){
        ...
        $grid .= "<table width='188%' border='0' class='table table-responsive table-bordered table-hover custab'>
		<thead>
			<tr class='bg-light'>
			<th width='2%' rowspan='2' class='align-middle text-left'>".$this->lang->line("sl")."</th>
			...
			<th width='48%' colspan='7' class='text-center'>".$this->lang->line("attendance")."</th>
			<th width='21%' colspan='3' class='text-center'>".$this->lang->line("salary_deduction")."</th>
			...
			<th width='".$bwidth."%' class='text-center'>".$this->lang->line("communication_allo")."</th>
        ...
        $i=1; $salary_id=0; $total_days=0; $working_day=0; $present=0; $full_leave=0; $holiday=0; $late=0; $early_out=0; $absent=0; $salary_day=0;
        $ttlbank_deduction=0; $ttlnet_bank_salary=0;
        foreach($query->result() as $row){
            $present=0; $full_leave=0; $holiday=0; $late=0; $early_out=0; $absent=0; $salary_day=0; $TotalCnB=0;
            $employee_id 	= $row->employee_id;
            $TotalCnB       = $row->gross_salary+$row->total_fix_payble;
			...
			if($CRES->num_rows() >0){
				...
				if($gross_deduction >0){
				  if($employee_id==209){ // Imran
				   $bank_deduction         = (($CRES->row()->absent_fine/100)*100);
				   $bank_deduction         = ($bank_deduction + $CRES->row()->loan + $CRES->row()->income_tax + $CRES->row()->pf);
				  }elseif($employee_id==211){ // Zannatul Ferdus
				    $bank_deduction         = (($CRES->row()->absent_fine/100)*100);
				    $bank_deduction         = ($bank_deduction + $CRES->row()->income_tax + $CRES->row()->pf);
				  }elseif($employee_id==251){ // Shahanaj Sultana
				    $bank_deduction         = (($CRES->row()->absent_fine/100)*60);
				    $bank_deduction         = ($bank_deduction + $CRES->row()->loan+ $CRES->row()->income_tax + $CRES->row()->pf);
				  }else{
				    $bank_deduction         = (($CRES->row()->absent_fine/100)*60);
				    $bank_deduction         = ($bank_deduction + $CRES->row()->income_tax + $CRES->row()->pf);
			...
			if($CRES->num_rows()>0 && $CRES->row()->bill_id >0 && $CRES->row()->status ==2 ){
			    ...
			    if($employee_id==209){ // Imran
				 $bank_absent_deduction = (($absent_deduction/100)*100); 
				}elseif($employee_id==211){ // Zannatul Ferdus
				 $bank_absent_deduction = (($absent_deduction/100)*100); 
				}else{
				 $bank_absent_deduction = (($absent_deduction/100)*60);   
				}
				$grid.= "
	...
	function ApprovedSalarySheet(){
		...
		foreach($squery->result() as $sdrow){
    		...
    		//====== Update Salary Sheet ========
    		$USQL= "UPDATE ".SALARY_SHEET_TBL." SET cash_deduction='".$cash_deduction."', cashover_deduction='".$cashover_deduction."' WHERE institute_id = $institute_id AND branch_id = $branch_id AND salary_sheet_id = $salary_id AND employee_id=$account_id";
    		$this->db->query($USQL);
    		//==== End Deduction ========
