...
class Department_model extends CI_Model {
	...
	function InsertRecord(){
        ...
        $department_name    =str_replace("U 0026", '&', $this->input->post('department_name'));
	...
	function FillRecord(){
   	...
   	function GetRecordGrid(){
		...
		$this->db->from(DEPARTMENT_TBL." AS d");
		...
		'<table width="100%"  border="0" class="table table-responsive table-bordered table-hover custab">
			<thead>
			  <tr class="active">
                ...
                <th width="22%">'.$this->lang->line("company_name").'</th>
			  	<th width="26%">'.$this->lang->line("department").' '.$this->lang->line("of").' '.$this->lang->line("name").'</th>
                <th width="26%">'.$this->lang->line("head_of_department").'</th>
			  ...
			  echo "<tr class='default'>
				...
				if($hasEditPM){
				echo "<span data-toggle='tooltip' data-placement='top' data-original-title='Edit'><a class='btn btn-info btn-xs' onclick=editRecord('".$row->department_id."') id='".$row->department_id."' href='#'><i class='fas fa-edit'></i></a></span>&nbsp;";
				}
				if($hasDelPM){
				echo "<span data-toggle='tooltip' data-placement='top' data-original-title='Delete'><a class='btn btn-danger btn-xs' data-toggle='modal' onclick=deleteRecord('".$row->department_id."') id='".$row->department_id."' data-target='#deleteModal'><i class='fa fa-trash'></i></a></span>";
				}
				echo "</td>
			  </tr>";
			  $i++;
