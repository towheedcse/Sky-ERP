<?php 
class SubsidiaryLedgerTwo_model extends CI_Model {
	...
	function InsertRecord(){
		...
		$sub_id		= $this->input->post('subsidiary-id');
		$group_id	= $this->input->post('group-head');
		
		$subsidiary_name2 = str_replace("U 0026", '&', $this->input->post('subsidiary-name2'));
		$subsidiary_name2 = $this->db->escape_str($subsidiary_name2);

		$created_by	= $this->session->userdata('created_by');
		
		$SQL= "INSERT INTO ".SUB_HEAD_L2_TBL."(company_id,sub_id,group_id,subsidiary_name2,created_by) VALUES('".$company_id."','".$sub_id."','".$group_id."','".$subsidiary_name2."','".$created_by."')";
		if($sub2_id ==""){
		$this->db->query($SQL);
		//print  $this->db->last_query();
		}else{
			$this->EditRecord($sub2_id);
		}
		//print  $this->db->last_query();
	   }
       	   function EditRecord($sub2_id){					
		$sub2_id	= $this->input->post('sub2-id');
		$company_id	= $this->input->post('company-name');			
		$sub_id		= $this->input->post('subsidiary-id');
		$group_id	= $this->input->post('group-head');
		...
		$SQL= "UPDATE ".SUB_HEAD_L2_TBL." SET company_id ='".$company_id."', sub_id ='".$sub_id."', group_id ='".$group_id."', subsidiary_name2 ='".$subsidiary_name2."',modified_by = '".$modified_by."', modified_time ='".$modified_time."' WHERE sub2_id = ".$sub2_id;
		$this->db->query($SQL);
		//print  $this->db->last_query();
       	  }
	   //============== Category Retrive by Ajax================
		...
		$group_id	= $this->input->post('src-top-head');
		$sub_id		= $this->input->post('src-sub-head');
		$from		= $this->input->post('from');
		$to		= $this->input->post('to');
		if($from==""){ $from=0;} if($to==""){ $to=50;}

		$this->db->select('c.*, s.sub_head_name, g.group_name');		
		$this->db->from(SUB_HEAD_L2_TBL.' AS c');					
		$this->db->join(SUB_HEAD_L1_TBL.' AS s', 's.sub_id=c.sub_id','LEFT');			
		$this->db->join(GROUP_HEAD_TBL.' AS g', 'g.group_id=s.parents_id','LEFT');
		if($group_id >0){
		   $this->db->where("c.group_id", $group_id);
		}
		if($sub_id >0){
		   $this->db->where("c.sub_id", $sub_id);
		}		
		$this->db->where("g.status", 1); // 1=Active
		$this->db->group_by('c.sub2_id');				
		$this->db->order_by('s.parents_id,c.sub_id,c.sub2_id','ASC');
		...
		'<table width="100%" id="data-table" class="table table-responsive table-hover table-bordered">
			<thead class="bg-primary">
			  <tr>
				...
				<th width="20%">'.$this->lang->line("gl")." ".$this->lang->line("name").'</th>
				<th width="30%">'.$this->lang->line("sl_level")."-".$this->lang->line("1")." ".$this->lang->line("name").'</th>
				<th width="33%">'.$this->lang->line("sl_level")."-".$this->lang->line("2")." ".$this->lang->line("name").'</th>
			...
			<td><strong>".$row->subsidiary_name2."</strong></td>
			<td align='center align-middle'>"; 
			if($hasEditPM){
			echo "<span data-toggle='tooltip' data-placement='top' data-original-title='Edit'> <a class='btn btn-info btn-xs' onclick=editRecord('".$row->sub2_id."') id='".$row->sub2_id."' href='#edit'><i class='fas fa-edit'></i></a></span> &nbsp;";
			}
			if($hasDelPM){
			echo "<span data-toggle='tooltip' data-original-title='Delete'><a class='btn btn-danger btn-sm' data-toggle='modal' onclick=deleteRecord('".$row->sub2_id."') id='".$row->sub2_id."' href='#deleteModal'><i class='fas fa-trash'></i></a></span>";
			}				
			echo "</td>
			</tr>";
			}
	...
	function GetTotalRecords(){
		...
		$this->db->where("g.status", 1); // 1=Active
		$this->db->group_by('c.sub2_id');						
		$this->db->order_by('c.sub2_id','ASC');
		$query = $this->db->get();
		if($query->num_rows() >0){
    ...
    function DeleteRecord(){
        ...
        $this->db->delete(SUB_HEAD_L2_TBL);
    }

    function FillRecord(){
        $sub2_id = $this->input->post('id');
        $this->db->select('*');
        $this->db->from(SUB_HEAD_L2_TBL);
        $this->db->where('sub2_id', $sub2_id);
        $query = $this->db->get();
        return $query->row();
    }
