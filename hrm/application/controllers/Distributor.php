...
class Distributor extends CI_Controller {	
	...
	function AddRecord(){ // Function Standard PascalCase
		...
		$this->Distributor_model->InsertRecord();
		$msg="Successfully save record!!!";
		$data['msg']=$msg;
		redirect(SERVER.'/distributor',$data);
	}
	function FillRecord(){
		$this->Site_model->has_menupermission($this->uri->segment(1));
		$erow = $this->Distributor_model->FillRecord();
		$arow = $this->Distributor_model->FillAccountHead();
		$password = substr($erow->password, 0, -2);
		echo $erow->distributor_id."##&##".$erow->company_id."##&##".$erow->branch_id."##&##".$erow->distributor_type."##&##".$erow->distributor_full_name."##&##".$erow->billing_address."##&##".$erow->shipping_address."##&##".$erow->contact_person."##&##".$erow->designation."##&##".$erow->gender."##&##".$erow->currency."##&##".$erow->attention_person."##&##".$erow->division."##&##".$erow->district."##&##".$erow->thana."##&##".$erow->address."##&##".$erow->permanent_address."##&##".$erow->mobile."##&##".$erow->phone."##&##".$erow->fax."##&##".$erow->email."##&##".$erow->login_id."##&##".$password."##&##".$arow->status;
	}
	function EditRecord(){
		$this->Site_model->has_menupermission($this->uri->segment(1));
		$this->Distributor_model->UpdateRecord();
	}
	function DelRecord(){
		$this->Site_model->has_menupermission($this->uri->segment(1));
		$this->Distributor_model->DelRecord();
	}	
	function GetRecord(){
		$this->Site_model->has_menupermission($this->uri->segment(1));
		$this->Distributor_model->GetRecordGrid();
	}
	...
	function ViewProfile(){
		...
		$this->load->view('print_distributor_profile',$data);
	}
	function GetProfile(){
		$menu_slug= $this->uri->segment(1);	
		$this->Site_model->has_menupermission($menu_slug);
