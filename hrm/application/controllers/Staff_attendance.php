...
class Staff_attendance extends CI_Controller {
	...
	function index(){
		...
		$this->load->view('staff_attendance',$data);
	}
	function GetStaffAttendanceRecord(){
		$this->Site_model->has_menupermission($this->uri->segment(1));	
		echo $this->Attendance_model->GetStaffAttendanceRecordGrid();
		
	}
	function SaveRecord(){ // Function Standard PascalCase
		$this->Site_model->has_menupermission($this->uri->segment(1));
		$this->Attendance_model->saveStaffAttendance();
		echo $this->Attendance_model->GetStaffAttendanceRecordGrid();
	}
	...
	function DailyAttendance(){
        ...
        $data['dquery']=$this->alllist->GetDepartmentList();
		$data['shquery']=$this->alllist->GetShiftList();
		$this->load->view('daily_attendance',$data);
	}
	function GetDailyAttendanceRecord(){
		$this->Site_model->has_menupermission($this->uri->segment(1));	
		echo $this->Attendance_model->GetDailyAttendanceRecordGrid();
	}
	...
	function EmpJobCard(){
		...
		$this->load->view('employee_job_card',$data);
	}
	function GetMonthlyJobCardRecord(){
		$this->Site_model->has_menupermission($this->uri->segment(1));	
		echo $this->Attendance_model->GetMonthlyJobCardRecord();
	}
	...
	function MyJobCard(){
		...
		$data['hasPrintOption'] = $this->Site_model->hasOptionPermission($menu_slug,"Print");
		$data['iquery']=$this->alllist->GetCompanyList();
		$this->load->view('my_job_card',$data);
	}
	function GetMyJobCardRecord(){
		echo $this->Attendance_model->GetMonthlyJobCardRecord();
	}
	...
	function MonthlyStaffAttendance(){
		...
		$data['dquery']=$this->alllist->GetDepartmentList();
		$data['shquery']=$this->alllist->GetShiftList();
		$this->load->view('monthly_staff_attendance',$data);		
	}
	
	function GetMonthlyStaffAttendanceRecord(){
		$this->Site_model->has_menupermission($this->uri->segment(1));	
		echo $this->Attendance_model->GetMonthlyStaffAttendanceList();
		
	}
	
	//************ ====Start Attendance Calender==== ************	     
    /********************* PUBLIC **********************/        
    /**
        ...
        $content='<div id="calendar">'.
                        ...
                        '<div class="box-content">'.
                                ...
                                for( $i=0; $i<$weeksInMonth; $i++ ){
                                    ...
                                    for($j=1;$j<=7;$j++){
                                        $content.=$this->_showDay($i*7+$j,$admission_id);
                                    }
                                }
    ...
    */
    private function _showDay($cellNumber){
        ...
        if( ($this->currentDay!=0)&&($this->currentDay<=$this->daysInMonth) ){
            ...
            $cellContent = "<span class='present'>".$this->currentDay." (P)</span>";
    ...
    */
	...
	function showStudentMonthlyAttendance(){ 
		echo $calendar = $this->show($admission_id=0);
	}
}
