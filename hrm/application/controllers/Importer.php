...
class Importer extends CI_Controller {	
	...
	function index(){
		...
		$data['ctquery']=$this->alllist->GetCurrencyList();
	...
	function AddRecord(){ // Function Standard PascalCase
		...
		$this->Importer_model->InsertRecord();
		$msg="Successfully save record!!!";
		$data['msg']=$msg;
		redirect(SERVER.'/importer',$data);
	}
	function FillRecord(){
		$this->Site_model->has_menupermission($this->uri->segment(1));
		$erow = $this->Importer_model->FillRecord();
		$arow = $this->Importer_model->FillAccountHead();
		$password = substr($erow->password, 0, -2);
		echo $erow->importer_id."##&##".$erow->company_id."##&##".$erow->branch_id."##&##".$erow->importer_type."##&##".$erow->importer_full_name."##&##".$erow->billing_address."##&##".$erow->shipping_address."##&##".$erow->contact_person."##&##".$erow->designation."##&##".$erow->gender."##&##".$erow->nationality."##&##".$erow->salesman_id."##&##".$erow->division."##&##".$erow->district."##&##".$erow->thana."##&##".$erow->present_address."##&##".$erow->permanent_address."##&##".$erow->mobile."##&##".$erow->phone."##&##".$erow->fax."##&##".$erow->email."##&##".$erow->currency."##&##".$erow->login_id."##&##".$password."##&##".$arow->status;
	}
	function EditRecord(){
		$this->Site_model->has_menupermission($this->uri->segment(1));
		$this->Importer_model->UpdateRecord();
	}
	function DelRecord(){
		$this->Site_model->has_menupermission($this->uri->segment(1));
		$this->Importer_model->DelRecord();
	}	
	function GetRecord(){
		$this->Site_model->has_menupermission($this->uri->segment(1));
		$this->Importer_model->GetRecordGrid();
	}
	...
	function ViewProfile(){
		...
		$importer_id	= $this->uri->segment(3);	
		$data['importer_id']= $importer_id;
		$this->load->view('print_importer_profile',$data);
	}
	function GetProfile(){
		$menu_slug= $this->uri->segment(1);	
		$this->Site_model->has_menupermission($menu_slug);
		...
		$this->Report_model->GetDistributorProfile();
	}	
