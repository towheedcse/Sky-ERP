...
<body class="hold-transition sidebar-mini">
    ...
    function IsPresent(attend_id,institute_id,branch_id,session_id,department_id,shift_id,employee_id,attendance_date,isChecked){
		...
		if(institute_id>0 && session_id>0 && shift_id>0 && employee_id>0 && attendance_date!=""){
		    $.ajax({
		        type: 'POST',
		        url: "<?php echo base_url();?>staff_attendance/SaveRecord",
		        data: "attend-id="+attend_id+"&company-id="+institute_id+"&branch-id="+branch_id+"&session-id="+session_id+"&department-id="+department_id+"&shift-id="+shift_id+"&employee-id="+employee_id+"&attendance-date="+attendance_date+"&in-time="+in_time+"&out-time="+out_time+"&present="+present,
    ...
    function isNullAndUndefined(variable){
 	...
 	$('.search').click(function(){
		...
		if(company_id >0 && session_id >0 && department_id >0 && shift_id>0 && attendance_date!=""){
		   $.ajax({
			type: 'POST',
