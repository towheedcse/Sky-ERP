...
<head>
    ...
    <title><?php echo $this->session->userdata('short_name');?>::<?php echo $this->lang->line("branch")." ".$this->lang->line("setup");?> </title>
...
<body class="hold-transition sidebar-mini">
        ...
        <div class="content-wrapper">
		       ...
		       <?php }?>
                  ...
                  <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                      ...
                      <li class="breadcrumb-item active"><?php echo $this->lang->line("branch");?></li>
                    </ol>
                  </div><!-- /.col -->
                </div><!-- /.row -->
              </div><!-- /.container-fluid -->
                ...
                <div class="container-fluid"> <!-- Start container-fluid -->
                    <div class="card card-primary show-create">
                        <div class="card-header">
                            <h3 class="card-title"><i class="fa fa-pen-square"></i> <?php echo $this->lang->line("new");?> <?php echo $this->lang->line("branch");?></h3>
		                ...
		                <div class="row">
		                    ...
		                    <div class="col-sm-4 col-md-4 col-lg-4">
		                        <div class="form-group required">
		                            <label class="control-label" for="branch_name"><?php echo $this->lang->line("branch_name");?></label>
		                            <input type="text" required="" placeholder="<?php echo $this->lang->line("branch_name");?>" name="branch_name" class="form-control text-capitalize" id="branch_name">
		                        </div>
		                    </div>
		                    <div class="col-sm-4 col-md-4 col-lg-4">
		                        <div class="form-group required">
		                            <label class="control-label" for="branch_address"><?php echo $this->lang->line("branch")." ".$this->lang->line("address");?>:</label>
		                            <input type="text" required="" placeholder="<?php echo $this->lang->line("branch")." ".$this->lang->line("address");?>" name="branch_address" class="form-control" id="branch_address">
		                        </div>
		                    </div>
		                </div>
		                <div class="row">
		                    ...
		                    <input type="hidden" name="branch_id" class="form-control" id="branch_id">
		                </div>
		                <div class="row">
		    ...
		    <?php if($hasViewOption){?>
                    <div class="card box-primary">
                        <div class="card-header">
                            <h3 class="card-title"><i class="fa fa-list"></i> <?php echo $this->lang->line("branch");?> <?php echo $this->lang->line("list");?></h3>
                            <div class="card-tools">
                              <button type="button" class="btn btn-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
                              <button type="button" class="btn btn-tool" data-widget="remove"><i class="fa fa-remove"></i></button>
                            </div>
	...
	var delId = $('#branch_id').val();
	$('#branch_id').val("");
	if(delId!=""){
	    $.ajax({
		type: 'POST',
		url: "<?php echo base_url();?>branch/DelRecord",
		data: "id="+delId,
	...
	function reloadDataGrid(){
		...
		$.ajax({
		    type: 'POST',
		    url: "<?php echo base_url();?>branch/GetRecord",
		    success: function(option){
	...
	function editRecord(id){
        $.ajax({
            type: 'POST',
            url: "<?php echo base_url();?>branch/FillRecord",
            data: "id="+id,
            success: function(option){
            //alert(option);
            rsStr = option.split("##&##");
            ...
            $('#branch_address').val(rsStr[3]);
            $('#phone').val(rsStr[4]);
            $('#mobile').val(rsStr[5]);
            $('#email').val(rsStr[6]);
            $('#branch_id').val(rsStr[0]);
	    $('.show-create').show();
            $('#alert').show();
	...
	$.ajax({
		type: 'POST',
		url: "<?php echo base_url();?>branch/AddRecord",
		data: "company_name="+companyName+"&branch_name="+branchName+"&branch_address="+branchAddress+"&phone="+phone+"&mobile="+mobile+"&email="+email+"&branch_id="+branchId,
		success: function(option){
		//alert(option);
		$('#company_name').val("");
		$('#company_name').trigger("chosen:updated");
