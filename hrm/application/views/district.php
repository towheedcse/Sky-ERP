...
<head>
    ...
    <title><?php echo $this->session->userdata('short_name');?>::<?php echo $this->lang->line("district")." ".$this->lang->line("setup");?></title>
...
<body class="hold-transition sidebar-mini">
        ...
        <div class="content-wrapper">
			...
			<?php }?>
                  ...
                  <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                      ...
                      <li class="breadcrumb-item active"><?php echo $this->lang->line("district");?></li>
                    </ol>
                  </div><!-- /.col -->
                </div><!-- /.row -->
              </div><!-- /.container-fluid -->
                ...
                <div class="container-fluid"> <!-- Start container-fluid -->
                    <div class="card card-primary show-create">
                        <div class="card-header">
                            <h3 class="card-title"><i class="fa fa-pen-square"></i> <?php echo $this->lang->line("new")." ".$this->lang->line("district");?></h3>
				    ...
				    <div class="col-sm-6 col-md-6 col-lg-6">
		                        <div class="form-group required">
		                            <label class="control-label" for="district_name"><?php echo $this->lang->line("district")." ".$this->lang->line("of")." ".$this->lang->line("name");?>:</label>
		                            <input type="text" required="" placeholder="<?php echo $this->lang->line("district")." ".$this->lang->line("of")." ".$this->lang->line("name");?>" name="district_name" class="form-control" id="district_name">
		                        </div>
		                    </div>
		    ...
		    <?php if($hasViewOption){?>
                    <div class="card box-primary">
                        <div class="card-header">
                            <h3 class="card-title"><i class="fa fa-list"></i> <?php echo $this->lang->line("district")." ".$this->lang->line("of")." ".$this->lang->line("list");?></h3>
                            <div class="card-tools">
                              <button type="button" class="btn btn-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
                              <button type="button" class="btn btn-tool" data-widget="remove"><i class="fa fa-remove"></i></button>
                            </div>
	...
	$('.confirm').click(function(){
	    	var delId = $('#district_id').val();
	   	$('#district_id').val("");
		if(delId!=""){
		    $.ajax({
		    type: 'POST',
		    url: "<?php echo base_url();?>district/DelRecord",
		    data: "id="+delId,
	...
	function reloadDataGrid(){
      ...
      $.ajax({
        type: 'POST',
        url: "<?php echo base_url();?>district/GetRecord",
        success: function(option){
	...
	function editRecord(id){
		$.ajax({
		type: 'POST',
		    url: "<?php echo base_url();?>district/FillRecord",
		    data: "id="+id,
		    success: function(option){
		    //alert(option);
		    rsStr = option.split("##&##");
		    ...
		    $('#district_name').val(rsStr[2]);
		    $('#status').val(rsStr[3]);
		    $('#status').trigger("chosen:updated");
		    $('#district_id').val(rsStr[0]);
		    $('.show-create').show();
		    $('#alert').show();
	...
	$('.save').click(function(){
		...
		$.ajax({
		    type: 'POST',
		    url: "<?php echo base_url();?>district/AddRecord",
		    data: "company_id="+company_id+"&district_name="+district_name+"&status="+status+"&district_id="+district_id,
		    success: function(option){
		    //alert(option);
		    $('#company_id').val("1");
		    $('#company_id').trigger("chosen:updated");
