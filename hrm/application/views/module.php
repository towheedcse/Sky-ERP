...
<head>
    ...
    <title><?php echo $this->session->userdata('short_name');?>::<?php echo $this->lang->line("module")." ".$this->lang->line("setup");?></title>
...
<body class="hold-transition sidebar-mini">
        ...
        <div class="content-wrapper">
		       ...
		       <?php }?>
                  ...
                  <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                      ...
                      <li class="breadcrumb-item active"><?php echo $this->lang->line("module");?></li>
                    </ol>
                  </div><!-- /.col -->
                </div><!-- /.row -->
              </div><!-- /.container-fluid -->
                ...
                <div class="container-fluid"> <!-- Start container-fluid -->
                    <div class="card card-primary show-create">
                        <div class="card-header">
                            <h3 class="card-title"><i class="fa fa-pen-square"></i> <?php echo $this->lang->line("new")." ".$this->lang->line("module");?></h3>
		                ...
		                <div class="row">
                            ...
                            <div class="col-sm-6 col-md-6 col-lg-6">
                                <div class="form-group required">
                                    <label class="control-label" for="module_name"><?php echo $this->lang->line("module_name");?>:</label>
                                    <input type="text" required="" placeholder="<?php echo $this->lang->line("module_name");?>" name="module_name" class="form-control text-capitalize" id="module_name">
                                </div>
                            </div>
		                </div>
		                <div class="row">
		                    <div class="col-sm-6 col-md-6 col-lg-6">
		                        <div class="form-group required">
		                            <label class="control-label" for="module_icon"><?php echo $this->lang->line("faicon");?>:</label>
		                            <input type="text" required="" placeholder="<?php echo $this->lang->line("faicon");?>" name="module_icon" id="module_icon" class="form-control">
		                        </div>
		                    </div>
		                    ...
		                    <div class="col-sm-3 col-md-3 col-lg-3">
		                        <div class="form-group required">
		                            <label class="control-label" for="module_status"><?php echo $this->lang->line("status");?>:</label>
		                            <div id="status_id">
		                                <select name="module_status" id="module_status" class="chosen-select" required="" placeholder="<?php echo $this->lang->line("status");?>">
		    ...
		    <?php if($hasViewOption){?>
                    <div class="card box-primary">
                        <div class="card-header">
                            <h3 class="card-title"><i class="fa fa-list"></i> <?php echo $this->lang->line("module")." ".$this->lang->line("of")." ".$this->lang->line("list");?></h3>
                            <div class="card-tools">
                              <button type="button" class="btn btn-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
                              <button type="button" class="btn btn-tool" data-widget="remove"><i class="fa fa-remove"></i></button>
                            </div>
	...
	$('.confirm').click(function(){
        var delId = $('#module_id').val();
        $('#module_id').val("");
        if(delId!=""){
            $.ajax({
            type: 'POST',
            url: "<?php echo base_url();?>module/DelRecord",
            data: "id="+delId,
	...
	function reloadDataGrid(){
        ...
        $.ajax({
            type: 'POST',
            url: "<?php echo base_url();?>module/GetRecord",
            success: function(option){
	...
	function editRecord(id){
        $.ajax({
            type: 'POST',
            url: "<?php echo base_url();?>module/FillRecord",
            data: "id="+id,
            success: function(option){
            //alert(option);
            rsStr = option.split("##&##");
            ...
            $('#module_name').val(rsStr[2]);
			$('#module_icon').val(rsStr[3]);
            $('#order_no').val(rsStr[4]);
            $('#module_status').val(rsStr[5]);
            $('#module_status').trigger("chosen:updated");
            $('#module_id').val(rsStr[0]);
	    $('.show-create').show();
            $('#alert').show();
	...
	$('.save').click(function(){
        ...
        $.ajax({
            type: 'POST',
            url: "<?php echo base_url();?>module/AddRecord",
            data: "company_id="+company_id+"&module_name="+module_name+"&order_no="+order_no+"&module_status="+module_status+"&module_icon="+module_icon+"&module_id="+module_id,
            success: function(option){
                //alert(option);
                $('#company_id').val("");
                $('#company_id').trigger("chosen:updated");
