...
<head>
    ...
    <title><?php echo $this->session->userdata('short_name');?>::<?php echo $this->lang->line("importer_po");?> </title>
    <style type="text/css">
        .btn-file {
            position: relative;
            overflow: hidden;
...
<body class="hold-transition sidebar-mini">
              ...
              <div class="container-fluid">
                <div class="row mb-2">
                  <div class="col-sm-6">
                    ...
                    <?php echo $this->lang->line("importer_po")." ".$this->lang->line("list");?>
                    </h1>
                  </div><!-- /.col -->
                  <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                      <li class="breadcrumb-item"><a href="<?php echo SERVER?>/dashboard/Userhome"><?php echo $this->lang->line("home");?></a></li>
                      <li class="breadcrumb-item active"><?php echo $this->lang->line("importer_po");?></li>
                    </ol>
                  </div><!-- /.col -->
                </div><!-- /.row -->
              </div><!-- /.container-fluid -->
            ...
            <?php if($hasViewOption){?>
                    ...
                    <div class="card box-primary">
                        <div class="card-header">
                            <h3 class="card-title"><i class="fa fa-search"></i> <?php echo $this->lang->line("search")." ".$this->lang->line("importer_po");?></h3>
							...
							<div class="row">
								...
								<div class="col-sm-4 col-md-4 col-lg-4">
									<div class="form-group">
										<label class="control-label" for="src-importer_id"><?php echo $this->lang->line("importer_name");?></label>
										<select name="src-importer_id" id="src-importer_id" class="chosen-select">
                    ...
                    <div class="card box-primary">
                        <div class="card-header">
                            <h3 class="card-title"><i class="fa fa-list"></i> <?php echo $this->lang->line("importer_po")." ".$this->lang->line("list");?></h3>
                            <div class="card-tools">
                              <button type="button" class="btn btn-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
                              <button type="button" class="btn btn-tool" data-widget="remove"><i class="fa fa-remove"></i></button>
                            </div>
	...
	function reloadDataGrid(){
        $.ajax({
            type: 'POST',
            url: "<?php echo base_url();?>distributorpo/GetImporterPOList",
	...
	$('.search').click(function(){
        ...
        var importerId      = $('#src-importer').val();
        ...
        $.ajax({
            type: 'POST',
            url: "<?php echo base_url();?>distributorpo/GetImporterPOList",
            data: "src-institute="+instituteId+"&src-branch="+branchId+"&src-importer="+importerId+"&src-po_no="+poNo+"&srcFrom="+srcFrom+"&srcTo="+srcTo,
            success: function(option){
                $('#dataGrid').html(option);
            }//Success
        });// ajax
    ...
    function nextPage(frm, to, pno) {
        ...
        $.ajax({
            type: 'POST',
            url: "<?php echo base_url();?>distributorpo/GetImporterPOList",
            data: "src-institute="+instituteId+"&src-branch="+branchId+"&src-importer="+importerId+"&src-po_id="+poId+"&srcFrom="+srcFrom+"&srcTo="+srcTo+"&from=" + frm + "&to=" + to + "&page_no=" + pno,
            success: function (option) {
            $('#dataGrid').html(option);
            }//Success
        });// End datagrid
