...
<head>
    ...
    <title><?php echo $this->session->userdata('short_name');?>::<?php echo $this->lang->line("area")." ".$this->lang->line("setup");?></title>
...
<body class="hold-transition sidebar-mini">
        ...
        <div class="content-wrapper">
			...
			<?php }?>
                  ...
                  <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                      ...
                      <li class="breadcrumb-item active"><?php echo $this->lang->line("area");?></li>
                    </ol>
                  </div><!-- /.col -->
                </div><!-- /.row -->
              </div><!-- /.container-fluid -->
                ...
                <div class="container-fluid"> <!-- Start container-fluid -->
                    <div class="card card-primary show-create">
                        <div class="card-header">
                            <h3 class="card-title"><i class="fa fa-pen-square"></i> <?php echo $this->lang->line("new")." ".$this->lang->line("area");?></h3>
                        ...
                        <div class="card-body">
                            ...
                            <form>
                                <div class="row">
                                    ...
                                    <div class="col-sm-6 col-md-6 col-lg-6">
                                        <div class="form-group required">
                                            ...
                                            <div id="div_id">
                                                <select name="division_id" id="division_id" class="chosen-select" required="">
                                ...
                                <div class="row">
                                    <div class="col-sm-6 col-md-6 col-lg-6">
                                        <div class="form-group required">
                                            <label class="control-label" for="area_name"><?php echo $this->lang->line("area").$this->lang->line("r")." ".$this->lang->line("name");?>:</label>
                                            <input type="text" required="" placeholder="<?php echo $this->lang->line("area").$this->lang->line("r")." ".$this->lang->line("name");?>" name="area_name" class="form-control text-capitalize" id="area_name">
                                        </div>
                                    </div>
                                    <div class="col-sm-6 col-md-6 col-lg-6">
                                        <div class="form-group required">
                                            <label class="control-label" for="area_status"><?php echo $this->lang->line("status");?>:</label>
                                            <div id="area_status_id">
                                                <select name="area_status" id="area_status" class="chosen-select" required="" placeholder="<?php echo $this->lang->line("status");?>">
                                    ...
                                    <input type="hidden" name="area_id" class="form-control" id="area_id">
                                </div>
                                <div class="row">
		    ...
		    <?php if($hasViewOption){?>
                    <div class="card box-primary">
                        <div class="card-header">
                            <h3 class="card-title"><i class="fa fa-list"></i> <?php echo $this->lang->line("area").$this->lang->line("r")." ".$this->lang->line("list");?></h3>
                            <div class="card-tools">
                              <button type="button" class="btn btn-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
                              <button type="button" class="btn btn-tool" data-widget="remove"><i class="fa fa-remove"></i></button>
                            </div>
	...
	$('.confirm').click(function(){
	    var delId = $('#area_id').val();
	    $('#area_id').val("");
        if(delId!=""){
            $.ajax({
            type: 'POST',
            url: "<?php echo base_url();?>area/DelRecord",
            data: "id="+delId,
	...
	function reloadDataGrid(){
	    $('#alert-delete').show();
            $.ajax({
		    type: 'POST',
		    url: "<?php echo base_url();?>area/GetRecord",
		    success: function(option){
	...
	function editRecord(id){
        $.ajax({
        type: 'POST',
            url: "<?php echo base_url();?>area/FillRecord",
            data: "id="+id,
            success: function(option){
            //alert(option);
            rsStr = option.split("##&##");
            ...
            $('#area_name').val(rsStr[3]);
            $('#area_status').val(rsStr[4]);
            $('#area_status').trigger("chosen:updated");
            $('#area_id').val(rsStr[0]);
	    $('.show-create').show();
            $('#alert').show();
	...
	$('.save').click(function(){
        ...
        $.ajax({
            type: 'POST',
            url: "<?php echo base_url();?>area/AddRecord",
            data: "division_id="+division_id+"&company_id="+company_id+"&area_name="+area_name+"&area_status="+area_status+"&area_id="+area_id,
            success: function(option){
            //alert(option);
            $('#division_id').val("");
            $('#division_id').trigger("chosen:updated");
