...
if (!function_exists('canAccess')) {
    function canAccess($slug = '')
    {
        ...
        // Sysadmin always bypasses all permission checks
        if ($userid == "imran") {
            return true;
        }

        // Return from session cache if available
        $cached = getFromSession('_acl_perms');
        if (is_array($cached)) {
            return isset($cached[$slug]);
        }
        ...
        $r1 = mysql_query(
            "SELECT p.slug FROM `permission` p
        ...
        // Store in session — use a sentinel key so empty arrays are cached too
        // (insertIntoSession rejects empty values, so we store directly)
        $_SESSION['_acl_perms'] = $perms;

        return isset($perms[$slug]);
    }
}
...
function getDeliveryPointName($delivery_pid)
{
    ...
    if ($delivery_pid != "") {
        ...
        if ($tfrow->details != "") {
            $delivery_point_name .= "<br>" . $tfrow->details;
        }
    }
