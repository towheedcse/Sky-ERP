<?php
function saveTopSizeSpection($sample_code,$totalsize){
   		for($x=0; $x<=9;$x++){
			$size_val ="";
			...
			if($size_spec_code!="" && $size_val!=""){
				$samplesize_spec_no = "TOP-".$sample_code.$size_spec_code;
				$size_val 			= substr($size_val, 0, -1);
				$type 				= "TOP";
				$created_by			= getFromSession('userid');
		    	$created_date       = date('Y-m-d h:i:s');
				
				$sql = "INSERT INTO ".SAMPLE_SIZE_SPEC_TBL." (`samplesize_spec_no`, `sample_code`, `type`, `size_spec_code`, `size_val`, `created_by`, `created_date`) VALUES ('$samplesize_spec_no', '$sample_code', '$type', '$size_spec_code', '$size_val', '$created_by', '$created_date')";
				mysql_query($sql);
			}
			
		}//end for		
   }
   function saveBottomSizeSpection($sample_code,$totalsize){
   		for($x=0; $x<=9;$x++){
			$size_val ="";
			...
			if($size_spec_code!="" && $size_val!=""){
				$samplesize_spec_no = "BOTTOM-".$sample_code.$size_spec_code;
				$size_val 			= substr($size_val, 0, -1);
				$type 				= "BOTTOM";
				$created_by			= getFromSession('userid');
		    	$created_date       = date('Y-m-d h:i:s');
